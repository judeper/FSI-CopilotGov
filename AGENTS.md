# Agent Instructions for FSI-CopilotGov

> **Keep in sync with `.github/copilot-instructions.md`.** The two files carry the same guidance — this one for AI tools that read `AGENTS.md` (Cursor, Aider, Claude Code, OpenAI agents, etc.), and `.github/copilot-instructions.md` for GitHub Copilot CLI. When you edit one, update the other.
...
- **MkDocs Material documentation site** in `docs/` (framework, 62 controls across 4 pillars, playbooks, reference).
- **Python assessment engine** (`assessment/engine/`) — scores collector evidence against the 62-control manifest and emits reports.
...
- **Controls** (`docs/controls/pillar-{1-4}-*/`) — 62 technical controls across 4 lifecycle pillars specifying *what* to configure and *why*.
