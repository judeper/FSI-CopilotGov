...
- **Solutions integration** (`assessment/data/solutions-lock.json`) — generated lock of the sister repo `FSI-CopilotGov-Solutions` main catalog, surfaced per-control in the SPA. Drift guarded by `scripts/check_solutions_drift.py`.
...
6. **`solutions-drift.yml`** — scheduled + PR: fails if `solutions-lock.json` drifts vs the sister repo main catalog.
