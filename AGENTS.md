...
FSI-CopilotGov is a governance framework for **Microsoft 365 Copilot** in US Financial Services. As of **v1.8.0**, the repository is **no longer documentation-only** — it now ships:

- **MkDocs Material documentation site** in `docs/` (framework, 64 controls across 4 pillars, playbooks, reference).
- **Python assessment engine** (`assessment/engine/`) — scores collector evidence against the 64-control manifest and emits reports.
...
- **Controls** (`docs/controls/pillar-{1-4}-*/`) — 64 technical controls across 4 lifecycle pillars specifying *what* to configure and *why*.
...
- **Reference** (`docs/reference/`) — regulatory mappings, glossary, admin toggles, FAQ, external tool references, **release-notes-v1.8.md**.
...
- Data stays client-side (no server calls). Portal export envelope is a versioned JSON blob (see `docs/reference/release-notes-v1.8.md`).
