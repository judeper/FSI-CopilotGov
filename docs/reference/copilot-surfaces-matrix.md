...
- **Copilot Chat (Basic):** Basic is web-grounded by default but can use explicitly supplied organizational content. Copilot-location DLP supports documented typed-prompt, web-search, and labeled-source paths; it doesn't scan the contents of files uploaded directly into prompts.
- **Copilot Search / Graph Connectors / SharePoint declarative agents / Copilot Studio agents:** Don't assume general DLP inspection of returned or generated content. Apply source-level controls and validate each explicitly documented Copilot or agent enforcement path.
- **Copilot Tuning:** Apply DLP and access controls to source data before tuning. Don't assume Copilot-location DLP scans tuned outputs; tuning-dataset governance and model-risk review require separate controls.
