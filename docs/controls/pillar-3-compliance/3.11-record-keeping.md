...
**Audit-trail alternative (Rule 17a-4(f)(2)):** As an alternative to non-rewriteable storage, a broker-dealer may instead preserve records in a manner that maintains an audit trail of all modifications, deletions, and access events throughout the record's retention period. This audit-trail alternative does not require WORM-formatted storage but requires a complete time-stamped audit trail for modifications and deletions, plus the other information needed to maintain authenticity and permit re-creation of the original record if modified or deleted. Preserve access events separately where required by firm policy, examination strategy, or counsel.

For Copilot content in Microsoft Purview, the audit-trail alternative may be supported through the following configuration, subject to counsel and records-management approval:
...
- Implement WORM-compliant storage for Copilot records per SEC Rule 17a-4(f) — either via **Option A** (audit-trail alternative: Purview regulatory record labels + Preservation Lock with documented compliance analysis per Rule 17a-4(f)(2)), or **Option B** (third-party WORM archival with vendor attestation)
- For **Option A** (audit-trail alternative per Rule 17a-4(f)(2)): leverage **SharePoint Preservation Hold Library** — provides audit trail of record creation and immutability when combined with retention hold; the system must automatically verify recording quality/accuracy, serialize and time-date stamp records, and support download to a readable format
...
- Conduct annual SEC 17a-4 compliance assessment for Copilot records with documented results, including review of whether the audit-trail alternative configuration satisfies Rule 17a-4(f)(2) requirements
...
| 6 | Verify 17a-4(f) compliance approach is documented | Option A (audit-trail alternative per Rule 17a-4(f)(2)) or Option B (WORM vendor attestation) is selected and documented | Recommended |
