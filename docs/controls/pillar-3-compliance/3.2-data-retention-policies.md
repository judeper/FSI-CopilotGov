...
- Enable **Preservation Lock** on retention policies governing regulated records only after compliance approval; this action is **irreversible**. Preservation Lock is configured in Security & Compliance PowerShell, not in the portal UI: run `Set-RetentionCompliancePolicy -Identity "<policy>" -RestrictiveRetention $true`, then verify `RestrictiveRetention` is `True` with `Get-RetentionCompliancePolicy`. Whether this configuration satisfies SEC Rule 17a-4(f) for the firm's records program remains a legal/compliance determination.
