...
> **Scope of verification:** The *Last Verified* date covers Microsoft product configuration guidance in this control (Purview DLP for the Microsoft 365 Copilot and Copilot Chat location, audit/DSPM surfaces, web-search controls, and associated cmdlets). SEC Reg S-P amendment existence, effective date, 72-hour service-provider notification wording, 30-day customer-notification outside timeframe, and the December 3, 2025 / June 3, 2026 compliance dates were checked against official SEC sources as factual references only. Applicability, customer-notification triggers, service-provider contract treatment, GLBA mapping, and examination posture remain firm/counsel determinations.
...
As of the 2026-09-24 verification date, the large-entity compliance date was December 3, 2025 and the small-entity compliance date was June 3, 2026. Institutions should verify their current compliance status against the amended Safeguards Rule requirements applicable to their entity size.
...
4. For Security & Compliance PowerShell creation, use the current Copilot location JSON and Copilot enforcement plane rather than workload-specific location switches:
    ...
    Create the rules with their alert configuration (`-GenerateAlert`, `-GenerateIncidentReport`, `-IncidentReportContent`, `-ReportSeverityLevel`) alongside their restriction actions (`-RestrictAccess`, `-RestrictWebGrounding`); a rule with no alert recipients produces no notification. Then run the fail-closed validation in [PowerShell Setup — Script 1b](../../playbooks/control-implementations/3.10/powershell-setup.md) before reporting the control as implemented: it stops on nonterminating cmdlet errors, resolves exactly one case-sensitive policy-name match before validating anything (a `$null` or non-existent `Identity` value makes `Get-DlpCompliancePolicy` return every policy in the tenant), and verifies `Mode=Enable`, the Copilot location GUID scoped tenant-wide with no exclusions, `EnforcementPlanes=CopilotExperiences`, both expected rules and their parent policy, their conditions and volume thresholds, their restriction actions, and their alert, incident report, severity, and recipient configuration. Where a property is not exposed or cannot be read, it stops and tells you what to verify manually in the portal.
