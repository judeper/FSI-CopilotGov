...
| **Microsoft 365 Chat (BizChat)** | High -- BizChat searches across all workloads and can surface NPI from any location | DLP for Copilot location; permission-based access controls; sensitivity labels |
...
- Deploy Copilot-specific DLP policies that restrict NPI surfacing in BizChat responses
...
3. Identify high-risk NPI access scenarios:
    - BizChat queries that could surface NPI from multiple sources
