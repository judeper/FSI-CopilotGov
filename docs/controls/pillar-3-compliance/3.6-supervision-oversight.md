...
Establish written supervisory procedures, qualified supervisor assignments, ongoing supervisory activities, and annual testing programs that address Microsoft 365 Copilot usage by associated persons. These controls provide evidence for FINRA Rule 3110 and SEC Regulation Best Interest review.
...
FINRA Regulatory Notice 24-09 reminds firms that existing supervisory obligations can apply to AI-generated content and AI tool use by associated persons. Applicability to the firm's Copilot deployment should be confirmed with counsel; human supervisory oversight remains the control design assumption.
...
|-----------|-----------|---------------------|
| **Written supervisory procedures** | 3110(b) | WSPs must specifically address Copilot usage policies, prohibited activities, review requirements, and escalation procedures; applicability: confirm |
| **Supervisory system** | 3110(a) | The overall system must account for Copilot where the firm treats it as a tool used in supervised activities; applicability: confirm |
| **Qualified supervisor** | 3110(a) | Designated supervisors should understand Copilot capabilities and limitations; registration and qualification expectations: confirm |
| **Review of communications** | 3110(b)(4) | Supervisory review must cover Copilot-assisted communications where the firm determines they are in scope; applicability: confirm |
| **Review of customer accounts** | 3110(b)(2) | Supervisory review of Copilot-assisted account activities |
| **Annual compliance meeting** | 3110(a)(7) | Annual meeting should address Copilot usage where the firm determines it is in scope; applicability: confirm |
| **Supervisory control system** | 3120 | Supervisory controls must test Copilot-specific procedures where approved by firm policy; applicability: confirm |
| **Annual testing** | 3120(b) | Annual testing must include Copilot-related supervisory procedures, including AI tool supervision effectiveness, when approved by firm policy; applicability: confirm |
| **Agent supervision** | 3110(a), 3120 | WSPs must address each deployed Copilot agent's scope, authorized actions, and supervisory review cadence where agents are in scope; applicability: confirm |
...
### FINRA 2026 Agentic AI Supervision and Microsoft 365 Copilot Agents

FINRA's evolving supervisory expectations signal that firms should assess whether FINRA Rule 3110(a) supervisory systems need to cover autonomous and semi-autonomous agent actions. Applicability to the following Microsoft 365 Copilot agent types: confirm.
...
**Declarative agents** built on the Microsoft 365 Copilot platform that access organizational data and respond to user queries should be assessed for WSP coverage. Each declarative agent should be documented in the WSP addendum with its authorized scope, the data sources it can access, and the supervisory review process for outputs it generates when the firm determines the agent is in scope. Agents that are given access to client data or produce client-facing content may carry higher supervisory exposure; applicability: confirm.
...
Institutions should prepare for FINRA examination scrutiny of agent supervision as part of the 2026 Annual Risk Monitoring and Examination Priorities cycle. Firms that have already extended their supervisory frameworks to cover Microsoft 365 Copilot agents and documented this coverage in their WSPs will be better positioned to demonstrate supervisory effectiveness.

### Full-Chain Telemetry for Agent Decision Reconstruction

When multiple agents or Copilot interactions contribute to a decision or output — for example, a declarative agent queries SharePoint data, summarizes it, and the associated person forwards the summary to a client — the firm should define what evidence is needed to reconstruct the full chain of AI interactions that led to the final output. This capability provides evidence for FINRA Rule 3110 supervisory documentation and FINRA Rule 3120(b) annual testing review; applicability: confirm.
...
2. **Agent-specific audit fields** — `AgentId` and `AgentName` (top-level), plus the nested `AccessedResources.XPIADetected` (cross-prompt injection attempt) flag within CopilotInteraction records identify agent-specific interactions
...
| **Copilot Notebooks** | Collaborative content that may be shared externally | External sharing controls + review requirement |
| **Teams channel agents** | Automated responses to user queries within Teams channels | Supervisory review per FINRA 3110(a); WSP coverage required; applicability: confirm |
...
| **Microsoft Scout (Frontier preview)** | Endpoint-executed agentic activity that can produce outputs contributing to client communications, correspondence, or recordkeeping. Supervisory review under FINRA Rule 3110 may apply where Scout outputs feed those surfaces. No official Communication Compliance support path for direct Scout local activity was found; supervise downstream Outlook, Teams, or document outputs where they enter monitored Microsoft 365 channels. Automation instructions and MCP-server output are stored locally on the endpoint and are outside the M365 DPA. | Supervise via the destination-surface Communication Compliance workflow; document the Scout-specific supervision scope under [Control 4.16](../pillar-4-operations/4.16-microsoft-scout-governance.md), including the local-artifact evidence gap. |
...
- Configure at least one Communication Compliance policy using the **Detect Microsoft Copilot interactions** template, or edit an existing policy and add current generative AI locations such as **Microsoft Copilot experiences**, **Enterprise AI apps**, and **Other AI apps** as applicable (**Purview portal > Communication compliance > Policies**). Copilot content is a location within standard CC policies -- no separate policy type is required
- Set communication direction to **Internal** for user-to-AI interaction monitoring
- Establish a post-use review sampling rate for Copilot-assisted correspondence via **CC policy > Review percentage > 1–100%** (default threshold: 5%; adjust to the firm-approved threshold with compliance/legal; portal-only configuration; not available via PowerShell)
...
- Increase post-use review sampling rate to the firm's approved threshold for Copilot-assisted communications (default threshold: 25%; adjust to the firm-approved threshold with compliance/legal); configure via **CC policy > Review percentage** in the Purview portal
- Implement Communication Compliance policies with automated flagging for supervisory review (see Control 3.4), including:
    - **Trainable classifiers and indicators:** Financial regulatory text, inappropriate content/images, sensitive information types, and generative AI indicators based on Azure AI Content Safety classifiers such as Prompt Shields and protected material detection
    - **Sensitive information types:** Up to 30 SITs per policy where documented for Communication Compliance policies created from Insider Risk Management policy indicators; verify the current SIT ceiling if creating the policy directly in Communication Compliance. Include FSI-relevant SITs (ABA Routing Number, U.S. Social Security Number, Credit Card Number, U.S. Bank Account Number)
...
- Assign Communication Compliance reviewer roles with appropriate permissions:
    - **Communication Compliance Admins:** Configure policies and settings, manage privacy settings and notice templates, and view/export policy updates
    - **Communication Compliance Analysts:** Access and investigate alerts
    - **Communication Compliance Investigators:** Access and investigate alerts, create message details reports, take advanced remediation actions (including escalation), and view Conversation/Translation tabs for a specific message
    - **Communication Compliance Viewers:** Access reports and view/export policy updates
...
- Escalate to 100% review sampling rate for priority users (executives, trading desk, M&A staff) via **CC policy > Review percentage**; adjust only to the firm-approved threshold
...
- Automate policy provisioning via PowerShell where supported:
    - **Modern Communication Compliance:** Policy creation and management is portal-only; PowerShell is not supported for creating or managing Communication Compliance policies
    - **Legacy:** `New-SupervisoryReviewPolicyV2` is still available in Security & Compliance PowerShell for legacy supervisory review scenarios, but it is not the standard provisioning path for modern Communication Compliance policies
    - **Limitation:** Sampling rate is portal-only and not configurable via PowerShell
...
2. Verify supervisors hold registrations approved by the firm's compliance/legal owners (Series 24, Series 26, or equivalent are examples to confirm with compliance/legal)
...
2. Set sampling rates:
    ...
    - Recommended: 25% of Copilot-assisted communications by default; adjust to the firm-approved threshold
    - Regulated: 100% of outbound Copilot-assisted client communications (pre-send review); adjust only to the firm-approved threshold
...
- [FINRA Annual Regulatory Oversight Report](https://www.finra.org/rules-guidance/guidance/reports/2025-finra-annual-regulatory-oversight-report)
