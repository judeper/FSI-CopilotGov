...
| **Microsoft Scout (Frontier preview)** | Endpoint-executed agentic activity that can produce outputs contributing to client communications, correspondence, or recordkeeping. Supervisory review under FINRA Rule 3110 may apply where Scout outputs feed those surfaces. **Communication Compliance is not supported for Scout activity** itself — coverage applies to the destination surface (e.g., Teams, Outlook) where Scout output is shared. Automation instructions and MCP-server output are stored locally on the endpoint and are outside the M365 DPA. | Supervise via the destination-surface Communication Compliance workflow; document the Scout-specific supervision scope under [Control 4.16](../pillar-4-operations/4.16-microsoft-scout-governance.md), including the local-artifact evidence gap. |
...
- Automate policy provisioning via PowerShell where supported:
    - **Modern:** `New-CommunicationCompliancePolicy` + `New-CommunicationComplianceRule` (requires E5 Compliance or Communication Compliance add-on; verify cmdlet availability with `Get-Command *CommunicationCompliance*`)
    - **Legacy:** `New-SupervisoryReviewPolicyV2` (still functional for supervisory review scenarios)
