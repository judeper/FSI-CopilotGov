...
For Copilot investigations, reviewers should create or open an eDiscovery case in the Microsoft Purview portal, select the custodian's Exchange mailbox as a source and build the search with **Add condition > Item class / Type > Contains any of > Copilot activity**. Microsoft documents user prompts and responses from AI applications as mailbox-stored items that are discoverable by the Copilot activity condition or by item class when a query string is required.
...
- Use **Purview portal > eDiscovery > [case] > Searches > Add condition > Item class / Type > Contains any of > Copilot activity** for the broadest AI-data query, with the custodian mailbox selected as a source. Add relevant SharePoint, OneDrive, Loop, or SharePoint Embedded locations separately when native files, pages, notebooks, or containers are in scope.
...
| 3 | Build an eDiscovery search with **Item class / Type > Contains any of > Copilot activity** against a custodian mailbox | Copilot and other AI application activity is included in the search results | Baseline |
