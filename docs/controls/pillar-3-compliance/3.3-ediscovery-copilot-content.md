# Control 3.3: Purview eDiscovery for Copilot-Generated Content
...
Configure Microsoft Purview eDiscovery to search, hold, review, and export Copilot-generated content across all Microsoft 365 workloads, supporting firm-managed regulatory examination responses, litigation discovery obligations, and internal investigation requirements where the firm determines the content is in scope.
...
SEC Rule 17a-4, FINRA Rule 4511, and the Federal Rules of Civil Procedure are commonly cited when firms evaluate whether Microsoft Copilot prompts, responses, drafts, pages, documents, or related metadata must be preserved or produced. Applicability, record classification, production format, and retention period are firm/counsel determinations; this control provides the Microsoft Purview technical pattern for finding, holding, reviewing, and exporting in-scope Copilot and AI-application data after that scope is defined.
...
For Copilot investigations, reviewers should create or open an eDiscovery case in the Microsoft Purview portal, select the custodian's Exchange mailbox as a source and build the search with **Add condition > Type > Contains any of > Copilot activity**. Microsoft documents user prompts and responses from AI applications as mailbox-stored items that are discoverable by the Copilot activity condition or by item class when a query string is required.
...
- Use **Purview portal > eDiscovery > [case] > Searches > Add condition > Type > Contains any of > Copilot activity** for the broadest AI-data query, with the custodian mailbox selected as a source. Add relevant SharePoint, OneDrive, Loop, or SharePoint Embedded locations separately when native files, pages, notebooks, or containers are in scope.
...
| **Microsoft Scout (Frontier preview)** — session and memory | Partial -- session and memory data stored in OneDrive are subject to OneDrive eDiscovery under existing tenant controls | Yes -- via OneDrive/mailbox hold on the user's account | Standard OneDrive/mailbox export workflows |
| **Microsoft Scout (Frontier preview)** — automation instructions and MCP-server output | **No** -- stored on the local endpoint, outside the Microsoft 365 DPA | **Not supported** by Purview eDiscovery | **Unsupported** -- document as a known evidence gap under [Control 4.16](../pillar-4-operations/4.16-microsoft-scout-governance.md); preservation of local artifacts must be arranged through endpoint-level mechanisms if required for investigation |
| **Microsoft Scout (Frontier preview)** — third-party inference | **No** -- inference through GitHub Copilot and third-party model providers is outside M365 residency, retention, and eDiscovery | **Not supported** by Purview eDiscovery | **Unsupported** -- record third-party inference boundary in vendor-risk artifacts (Control 1.10) |
...
| 3 | Build an eDiscovery search with **Type > Contains any of > Copilot activity** against a custodian mailbox | Copilot and other AI application activity is included in the search results | Baseline |
...
- [Microsoft Purview eDiscovery — notable changes (classic experiences retired August 31 2025)](https://learn.microsoft.com/en-us/purview/edisc#notable-changes-in-ediscovery)
