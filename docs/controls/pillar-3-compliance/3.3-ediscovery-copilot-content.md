...
### Unified Purview eDiscovery Portal Transition (2026)

Microsoft is retiring the classic Content Search and eDiscovery (Standard) experiences and merging them into the **unified Purview eDiscovery portal**. Organizations still using the legacy Content Search tools in the Microsoft Purview compliance portal should plan their transition to the unified eDiscovery experience.

- **What is changing:** The standalone Content Search interface and the separate eDiscovery (Standard) portal are being deprecated. All search, hold, review, and export workflows are consolidated into the unified eDiscovery case management interface at **Microsoft Purview > eDiscovery > Cases**
- **Deprecated PowerShell cmdlets:** eDiscovery export PowerShell cmdlets associated with the classic experience are being retired. Organizations that rely on scripted export workflows should migrate to the new unified portal workflow, which uses in-browser export and download capabilities
- **Migration action:** Inventory any existing PowerShell scripts or automation that use deprecated eDiscovery export cmdlets (such as `New-ComplianceSearchAction -Export`) and plan replacements using the unified portal's export workflow or updated cmdlets. Organizations should verify that all current eDiscovery workflows function correctly in the unified portal before the classic experience is fully retired

### Case-Centric Access Model

Searches performed outside of formal cases (previously known as standalone Content Searches) are now accessible within a special **"Content Search" case** in the unified portal. This architectural change improves access control and auditability by centralizing all search activities within the case framework.

- **FSI governance impact:** All eDiscovery searches — including exploratory or ad hoc searches — are now tracked within the case management structure, providing a clearer audit trail for regulatory examinations. FINRA and SEC examiners requesting evidence of search activities can be provided with case-level access logs rather than fragmented search histories
- **Access control:** The case-centric model enables role-based access to search results. Compliance personnel who are members of a specific case can access that case's searches and results, while personnel outside the case cannot — improving information segregation during sensitive investigations
...
!!! warning "Emerging Surface — Verify Discoverability"

    M365 Copilot memory persists user context across conversations. This content may be relevant to litigation holds and regulatory inquiries, and its discoverability through Purview eDiscovery should be assessed.

**eDiscovery implications for FSI:**

- **Content discoverability:** Organizations should verify whether Copilot memory entries are searchable through Purview eDiscovery content search and eDiscovery Premium cases. If memory content is not indexed for eDiscovery, firms face a potential gap in responsive data collection for FINRA 8210 requests and civil litigation.
- **Custodian holds:** When placing custodians on legal hold, organizations should confirm whether the hold extends to Copilot memory content or whether separate preservation actions are needed.
- **Data mapping:** Copilot memory should be included in the organization's data map for litigation readiness. The Federal Rules of Civil Procedure Rule 26(f) conference obligations include identifying all relevant data sources, which may now include Copilot memory.
- **Cross-reference:** Organizations should coordinate with [Control 3.2 — Data Retention](3.2-data-retention-policies.md) to ensure memory content is both retained and discoverable.

**Recommended actions:**

1. Test whether Copilot memory content appears in Purview eDiscovery search results
2. Include Copilot memory in litigation readiness checklists and data source inventories
3. Monitor Microsoft documentation for updates on memory content discoverability
