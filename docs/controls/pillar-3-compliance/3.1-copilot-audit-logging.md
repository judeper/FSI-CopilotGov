...
<!-- NOTE: JailbreakDetected and XPIADetected fields are based on emerging audit capabilities. Verify field availability in your tenant before building automated workflows. -->
| `JailbreakDetected` | Boolean indicating whether a jailbreak attempt was detected | Security event requiring immediate escalation per 12 CFR part 30, appendix D (OCC Heightened Standards) and FFIEC incident response expectations |
...
Current Microsoft Learn guidance records Agent 365 and Copilot Studio activity through documented operations rather than older, tenant-specific labels. Use the `Search-UnifiedAuditLog` `-Operations` parameter when searching for agent activity, and constrain the record type to documented `AuditLogRecordType` enum members such as `CopilotInteraction` (261) for Microsoft 365 Copilot interactions (including Copilot Studio agent interactions) or `CopilotAgentManagement` (384) for Microsoft 365 Copilot agent admin events.
...
**Microsoft 365 Copilot agent management operations** — `DeployedAgent`, `BlockedAgent`, `UnblockedAgent`, `UpdatedAgent`, `RemovedAgent`, `DeletedAgent`, and `UpdatedTenantSettings` capture agent deployment, blocking, manifest updates, removal, deletion, and tenant-level Copilot-agent settings changes. These operations help provide the change-management audit trail needed for Sarbanes-Oxley §§302/404 IT general controls, where applicable to ICFR, and FINRA Rule 3110 supervisory review.

**Copilot Studio authoring and lifecycle operations** — Copilot Studio events appear under the Copilot Studio workload with documented event labels such as `BotCreate`, `BotDelete`, `BotUpdateOperation-BotNameUpdate`, `BotUpdateOperation-BotAuthUpdate`, `BotUpdateOperation-BotPublish`, `BotUpdateOperation-BotShare`, `BotComponentCreate`, `BotComponentUpdate`, `BotComponentDelete`, `AIPluginOperationCreate`, `AIPluginOperationUpdate`, `AIPluginOperationDelete`, `EnvironmentVariableCreate`, `EnvironmentVariableUpdate`, and `EnvironmentVariableDelete`. Organizations should verify these operations in their tenant's Purview Audit results before building automated workflows.
...
> activities — including `CopilotInteraction`, `TeamCopilotInteraction`, and
> `CopilotAgentManagement` — are retained for 180 days by default**, and
...
- **Per-workload cost tracking:** Tag audit event streams by record type and operation (`CopilotInteraction`, `CopilotAgentManagement`, `TeamCopilotInteraction`, Agent 365 operations, and Copilot Studio lifecycle operations) to attribute retention costs to the appropriate business unit
...
**Microsoft 365 Copilot agent management (deploy, block, update, remove, tenant settings):**

```
Search-UnifiedAuditLog -StartDate "2026-01-01" -EndDate "2026-01-31" -RecordType CopilotAgentManagement -Operations "DeployedAgent","BlockedAgent","UnblockedAgent","UpdatedAgent","RemovedAgent","DeletedAgent","UpdatedTenantSettings" -ResultSize 5000
```

**Copilot Studio authoring and lifecycle events:**

```
Search-UnifiedAuditLog -StartDate "2026-01-01" -EndDate "2026-01-31" -Operations "BotCreate","BotDelete","BotUpdateOperation-BotPublish","BotComponentCreate","BotComponentUpdate","BotComponentDelete","EnvironmentVariableCreate","EnvironmentVariableUpdate","EnvironmentVariableDelete" -ResultSize 5000
```
...
| **Copilot Agents (declarative and Copilot Studio)** | CopilotInteraction, CopilotAgentManagement, Agent 365 operations | Agent invocations, tool calls, deploy/block/update/remove changes, Copilot Studio publishing and component updates |
...
- Set audit log retention to a minimum of 180 days — note E3/Audit (Standard) default is **180 days for all activities**; E5/Audit (Premium) provides an unmodifiable **1-year default only for Exchange Online, SharePoint, OneDrive, and Microsoft Entra audit records**, while `CopilotInteraction`, `CopilotAgentManagement`, and `TeamCopilotInteraction` continue to use the **180-day default** unless a custom audit retention policy extends them (extendable to **10 years** via the Audit Premium 10-year add-on)
...
- Upgrade to Audit (Premium) (E5 license tier) and create custom audit retention policies for Copilot record types — Audit (Premium) only extends the default retention to 1 year for Exchange/SharePoint/OneDrive/Entra audit records; `CopilotInteraction`, `CopilotAgentManagement`, and `TeamCopilotInteraction` remain on the 180-day default until a custom policy is created
...
- Integrate Copilot audit events with Microsoft Sentinel or third-party SIEM — verify whether the connector forwards `CopilotAgentManagement` events and documented Agent 365 operations; use the Office 365 Management Activity API or an alternative ingestion method where connector coverage is incomplete
...
- Create standardized audit search queries and save them for recurring use — include queries for `CopilotInteraction`, `TeamCopilotInteraction`, admin operations (`UpdateTenantSettings`, `CreatePlugin`, `DeletePlugin`, `EnablePromptBook`), Agent 365 operations (`AIInvokeAgent`, `AIExecuteTool`, `AIInferenceCall`), Microsoft 365 Copilot agent management (`DeployedAgent`, `BlockedAgent`, `UpdatedAgent`, `RemovedAgent`, `UpdatedTenantSettings`), and Copilot Studio operations (`BotCreate`, `BotDelete`, `BotComponentUpdate`, `BotComponentDelete`, `BotUpdateOperation-BotPublish`, `EnvironmentVariableUpdate`)
- Configure recurring searches for Copilot Studio events and Agent 365 operations using the `-Operations` parameter; configure search queries that target the `CopilotAgentManagement` (384) record type for Microsoft 365 Copilot agent admin operations (deploy/block/update/remove/tenant settings) — these record types use the 180-day default retention on both Standard and E5/Audit (Premium); a custom audit retention policy is required to extend them up to the 10-year Audit Premium maximum
...
- Implement real-time audit event streaming to SIEM for immediate compliance monitoring — supplement the M365 Sentinel connector with Office 365 Management Activity API ingestion for `CopilotAgentManagement` events and Agent 365 operations when needed
...
4. Click **Save**
5. Create a second policy for Microsoft 365 Copilot agent management events:
    - **Name:** `FSI-Agent-Admin-Audit-Retention-7Year`
    - **Record types:** `CopilotAgentManagement` (Microsoft 365 Copilot agent deploy/block/update/remove and tenant-level Copilot agent settings — RecordType 384 in the AuditLogRecordType enum) and `TeamCopilotInteraction` (Teams Copilot AI Notes / Live Notes / Meeting Moderator — RecordType 334)
    - **Operations to validate in search procedures:** `AIInvokeAgent`, `AIExecuteTool`, `AIInferenceCall`, `DeployedAgent`, `BlockedAgent`, `UnblockedAgent`, `UpdatedAgent`, `RemovedAgent`, `DeletedAgent`, `UpdatedTenantSettings`, `BotCreate`, `BotDelete`, `BotComponentCreate`, `BotComponentUpdate`, `BotComponentDelete`, `BotUpdateOperation-BotPublish`, `EnvironmentVariableCreate`, `EnvironmentVariableUpdate`, `EnvironmentVariableDelete`
...
2. Enable the **Microsoft 365** data connector for Exchange, SharePoint, and Teams audit events (populates the `OfficeActivity` table; this connector does **not** deliver `CopilotInteraction` records)
3. Enable the dedicated **Microsoft Copilot** data connector (Preview) to ingest Microsoft 365 Copilot and Security Copilot audit events into the `CopilotActivity` table
4. Create analytics rules for Copilot-specific alert scenarios (see [Control 4.11 — Sentinel Integration](../pillar-4-operations/4.11-sentinel-integration.md) for full KQL templates against `CopilotActivity`)
