...
Communication Compliance can analyze prompts and responses for Microsoft 365 Copilot, Copilots built with Microsoft Copilot Studio, AI applications connected by Microsoft Entra or Microsoft Purview Data Map connectors, and other AI apps detected from browser or network activity. For Microsoft 365 Copilot and Copilot Chat deployments covered by this framework, configure Communication Compliance policies as follows. Configuration guidance for non-Microsoft 365 AI data is outside the scope of this framework and generally requires pay-as-you-go billing.

### Recent Enhancements (April 2026)

**Content preview in IRM alerts:** Insider Risk Management alerts now support previewing relevant files referenced from Activity explorer without first creating a case, subject to permissions. This is an IRM capability (preview April 2026, GA July 2026), not a Communication Compliance settings toggle. Organizations should verify that preview access is restricted to authorized investigators per the firm's information barrier and need-to-know policies.

**Case creation without content:** Insider Risk Management can create cases without downloading content (preview February 2026, GA May 2026), with a 2,000 active-case limit and the existing 100 active content-download limit unchanged. This supports workflows where the compliance team needs to initiate tracking, assign ownership, or begin documentation before full content review.

**Pay-as-you-go model for AI-driven indicators:** Certain AI-driven communication compliance indicators -- particularly those covering non-Microsoft 365 AI applications -- operate under a **pay-as-you-go billing model**. Microsoft 365 Copilot communication compliance indicators require eligible Communication Compliance licensing such as Microsoft Purview Suite (formerly Microsoft 365 E5 Compliance), Office 365 E5, or Office 365 E3 with the Advanced Compliance add-on, but do not require PAYG billing. Organizations should review their billing configuration to understand which indicators incur PAYG charges, particularly if monitoring third-party AI tools alongside Microsoft 365 Copilot.

**Generative AI app categories:** Communication Compliance supports generative AI categories including **Microsoft Copilot experiences**, **Enterprise AI apps**, and **Other AI apps**. Examples of non-Microsoft 365 AI data include Fabric Copilot, Security Copilot, Copilot Studio, connected or cloud AI apps, and AI apps detected through browser activity; these generally require PAYG billing. Organizations should verify which categories are in scope for their supervisory procedures and document any platforms not yet covered.

### Built-In Copilot Interactions Policy Template

Microsoft Purview Communication Compliance includes a built-in **Detect Microsoft Copilot interactions** policy template that pre-configures monitoring for Copilot-assisted communications. This template provides a faster path to baseline coverage than building a custom policy from scratch.

| Template Feature | Description |
|-----------------|-------------|
| **Pre-configured scope** | Targets Microsoft 365 Copilot and Microsoft 365 Copilot Chat interactions; existing policies can also add Microsoft Copilot experiences, Enterprise AI apps, and Other AI apps as generative AI locations |
| **Built-in classifiers** | Enables Microsoft-provided classifiers for regulatory compliance, protected material detection, and Prompt Shields |
...
1. Navigate to **Microsoft Purview portal > Communication compliance > Policies**
2. Select **+ Create policy** > **Detect Microsoft Copilot interactions**
3. Configure the supervised user scope (recommended: all Copilot-licensed users for Baseline; registered representatives and advisors for targeted deployments)
4. Assign reviewers (minimum two for redundancy)
5. Review the pre-selected classifiers and add any firm-specific keyword dictionaries
6. Enable the policy and monitor the dashboard for initial results
...
| **Microsoft 365 Copilot Chat** | Yes | Microsoft Copilot experiences | Microsoft 365 Copilot and Microsoft 365 Copilot Chat interactions can be detected by Communication Compliance policies; retention and eDiscovery specifics are handled in Controls 3.2 and 3.3 |
| **Word Copilot** | Indirect | Monitored channels | Communication Compliance is message/prompt-response oriented. Do not assume generated document content is reviewed unless it is sent or shared through a monitored channel; use DLP, sensitivity labels, and document review controls for document content |
...
| **Security Copilot / Fabric Copilot / Copilot Studio** | Awareness only | Microsoft Copilot experiences or non-Microsoft 365 AI data categories, depending on surface | Coverage exists for these surfaces; configuration guidance is outside scope of this framework, and non-Microsoft 365 AI data generally requires PAYG billing |
...
Configure Communication Compliance indicators from Insider Risk Management:
...
3. For custom Communication Compliance policy alerts as IRM indicators, select the relevant CC policies to include. Current roadmap communications describe this custom-policy indicator capability as GA in November 2025.
4. Save the configuration and confirm at least one active IRM policy targets the affected users.

When selecting **Create policy** from the IRM indicator setup flow, Microsoft can create a dedicated Communication Compliance policy such as `Insider risk indicator <timestamp>`. Once enabled, CC policy alerts can contribute to IRM alerts. Review the IRM dashboard (Control 2.10) to confirm indicators are flowing after policy matches occur.
...
4. Configure:
    ...
    - **Locations:** Exchange Online, Microsoft Teams, and Microsoft Copilot experiences
...
1. Go to **Insider Risk Management** > **Settings** > **Policy indicators**
2. Enable the Communication Compliance indicators that should contribute to IRM policies
3. Select which CC policy alerts generate IRM risk indicators (start with high-severity regulatory compliance and MNPI violations)
4. Verify in the IRM dashboard (Control 2.10) that CC indicators are flowing after policy matches occur
...
!!! warning "Emerging Surface -- Monitor for transcript and record coverage"

    Microsoft is shipping voice-based Copilot experiences across multiple apps and channels. Voice-based Copilot interactions introduce communication compliance challenges that organizations should prepare for, but organizations should verify transcript, record, and retention behavior for each specific voice feature before relying on Communication Compliance or retention coverage.
...
2. Assess whether existing communication compliance policies will apply to voice transcripts or other persisted records for each feature
...
- [Trainable classifiers in Microsoft Purview](https://learn.microsoft.com/en-us/purview/trainable-classifiers-learn-about)
