...
|---------|---------|-----------|-----------|----------------------|--------------------------|
| **Copilot Pages** | User-owned SharePoint Embedded container shared with Copilot Notebooks and Loop My workspace; admin/audit application name is `Loop` | High — collaborative edits | Tenant-verify available copy/branch behavior and audit evidence | Shared/linkable as Loop-backed content in supported hosts | Microsoft 365 retention policy scoped via **All SharePoint Sites** or the specific SharePoint Embedded container; `.page` audit/filter evidence |
| **Copilot Notebooks** | User-owned SharePoint Embedded container shared with Copilot Pages and Loop My workspace; `.pod` files manage notebook content | High — collaborative edits and notebook updates | No separate Pages/Notebooks application filter; tenant-verify copy/duplicate behavior | Shared through supported Copilot/Loop experiences | Microsoft 365 retention policy scoped via **All SharePoint Sites** or the specific SharePoint Embedded container; no end-user recycle bin for individually deleted Notebooks |
| **Loop components** | SharePoint Embedded, SharePoint sites, or OneDrive depending on where created | High — live, real-time co-edit | Duplication/copy behavior depends on host and tenant experience | Yes — embedded in Teams, Outlook, Word, Whiteboard, and other supported hosts | Retention policy on the underlying storage location; audit searches by Loop application IDs and `SourceFileExtension` values such as `.loop`, `.page`, `.pod`, or legacy `.fluid` |
...
### Tenant Verification Required for Lineage Claims

Microsoft documents the storage and compliance surfaces for Copilot Pages and Copilot Notebooks, but branch/copy event names, host-reference completeness, and legal-hold container picker availability can vary by tenant rollout. Verify in your tenant before relying on lineage automation: **SharePoint admin center > SharePoint Embedded > Active containers > copy container URL**; **Purview > Audit > search for `page`, `loop`, `loot`, `fluid`, or the Loop application IDs**; export results and filter by `SourceFileExtension` (`page`, `loop`, `pod`, `fluid`); then reconcile the exported events to the retained `.page`, `.loop`, and `.pod` items.
...
|---|------------------|------------------|------------------|
| 1 | Confirm Copilot Pages, Copilot Notebooks, and Loop components are within scope of a Microsoft 365 retention policy | Retention policy explicitly covers OneDrive and SharePoint sites hosting these artifact types | Baseline |
...
| 12 | Confirm Pages, Notebook, and Loop holds integrate with the eDiscovery and legal hold workflow | Test evidence documents whether branch lineage is preserved within hold scope | Regulated |

## Additional Resources

- [Microsoft 365 Copilot overview](https://learn.microsoft.com/en-us/microsoft-365/copilot/)
- [Manage Microsoft Loop in your organization](https://learn.microsoft.com/en-us/microsoft-365/loop/loop-admin-configuration)
- [Loop component governance and Microsoft 365 storage](https://learn.microsoft.com/en-us/microsoft-365/loop/loop-components-teams)
- [Retention for SharePoint and OneDrive in Microsoft Purview](https://learn.microsoft.com/en-us/purview/retention-policies-sharepoint)
- [Microsoft Purview retention policies overview](https://learn.microsoft.com/en-us/purview/retention)
- [Microsoft Purview unified audit log search](https://learn.microsoft.com/en-us/purview/audit-search)
