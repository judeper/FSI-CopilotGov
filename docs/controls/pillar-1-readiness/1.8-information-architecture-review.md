...
| **Admin enablement** | Microsoft enables Anthropic models (the AI sub-processor underlying parts of AI in SharePoint) **on by default for most customers in commercial cloud, excluding EU/EFTA and UK**. The previous tenant-admin PowerShell opt-in under Anthropic's separate commercial terms has been **deprecated**; access is now governed under the Microsoft Product Terms and Data Protection Addendum. Admins can disable Anthropic models where their risk posture requires it. |
| **Cloud availability** | Not available in government or sovereign cloud environments; EU/EFTA and UK customers are excluded from the default-on rollout |
...
- **Sub-processor governance:** Because Anthropic models are enabled by default for most commercial tenants, FSI organizations should proactively review this external AI sub-processor under their third-party risk management framework and **disable it where their risk posture requires**, rather than assuming it is off. Organizations should verify that the sub-processor arrangement meets their data handling and residency requirements before relying on it.
- **Recommended approach:** FSI organizations should confirm the current Anthropic enablement state for their tenant, evaluate AI in SharePoint in a non-production environment before broad reliance, and establish a metadata review process for AI-generated content before it enters the Copilot grounding scope.
...
| **Metadata governance policy** | Is there a documented policy for required metadata fields per site or library type? | Without policy, metadata population degrades over time as new content is created without classification |
