...
**Regulatory Reference:** GLBA §501(b), Sarbanes-Oxley §§302/404 (where applicable to ICFR), FFIEC IT Examination Handbook (Access Control), SEC Regulation S-P
...
Conduct a comprehensive audit of user permissions across all Microsoft 365 workloads that Copilot accesses via Microsoft Graph -- including SharePoint Online, OneDrive for Business, Exchange Online, Microsoft Teams, and Graph API permissions -- to identify and remediate permission sprawl, overly broad access, and stale permissions before Copilot deployment. This control supports the firm's least-privilege governance decisions and helps prevent Copilot from surfacing content that users have access to but should not.

---

## Why This Matters for FSI

- **GLBA §501(b):** Legal/compliance should determine whether customer-information safeguards require specific permission-audit evidence before or during Copilot deployment.
- **Sarbanes-Oxley §§302/404 (where applicable to ICFR):** ICFR owners should determine whether access to financial systems and data belongs in the firm's SOX control evidence when Copilot can surface content across accessible workloads.
- **FFIEC IT Examination Handbook (Access Control):** Risk and technology owners should determine how least privilege, separation of duties, and periodic access-review expectations apply to Copilot-accessible Microsoft 365 workloads.
- **SEC Regulation S-P:** Privacy owners should determine whether permission controls for consumer financial information need Copilot-specific testing or evidence.
- **FINRA Rule 3110 (supervisory systems and WSPs):** Broker-dealer compliance should determine whether Copilot's changes to information discovery affect supervisory systems, written supervisory procedures, or access-pattern review.
...
Current Microsoft Purview Data Security Posture Management uses broader DSPM role groups plus AI-specific roles. These roles are in addition to existing SharePoint and Exchange administrative roles and should be assigned as part of the Copilot permission model configuration. Assigning these roles supports the firm's separation of duties between general IT administration and AI compliance oversight, with FFIEC IT Examination Handbook role-based access control references treated as applicability prompts for risk and compliance review.

| Role or role group | Description | Use Case |
|--------------------|-------------|----------|
| **Purview Compliance Admin** / **Data Security Management** | Manage current DSPM and Microsoft Purview data security solutions, including DLP, information protection, insider risk, posture insights, and guided remediation workflows. Use the least-privileged role group that matches the task. | DSPM configuration, remediation workflow ownership |
| **Data Security Viewers** role group / **Data Security Viewer** role | View current DSPM insights and use Security Copilot to view detailed data security posture information. | Executive reporting, posture monitoring |
...
| **Purview Data Security AI Viewer** / **Data Security AI Viewers** role group | Read-only access to AI-related data security information in current DSPM and DSPM for AI (classic). Does not expose prompt or response content. | AI observability monitoring, compliance reporting |
| **Purview Data Security AI Content Viewer** / **Data Security AI Content Viewers** role group | Provides read-only access to AI interaction prompt and response details where supported. In current DSPM, viewing prompts and responses within **AI Interaction** events also requires **Content Explorer Content Viewer**. | Compliance investigation, DLP match review |
| **Data Security AI Admins** (Data Security AI Admin role) | Provides editing capabilities for DLP policies related to Copilot and viewing AI content in DSPM. Microsoft documents that this role **does not** have access to read AI interaction prompts and responses. | Copilot DLP policy governance, AI data security administration |
...
- **Data Security Viewers**, **Data Security AI Viewers**, **Data Security AI Content Viewers**, **Data Security AI Admins**, and **Data Security Management**: assigned via Microsoft Purview portal > Settings > Roles and scopes > Role groups; verify exact names in your tenant because Purview displays role and role-group names separately
...
| **Recommended** | All Baseline requirements plus: extend audit to OneDrive sharing, Teams membership, and Exchange delegation. Remediate all EEEU access on all sites. Review and reduce large group memberships (>100 members). Implement quarterly permission audit cadence. Assign Purview Data Security AI Content Viewer plus Content Explorer Content Viewer to investigation team members authorized to review AI Interaction prompt and response content where current DSPM requires both roles. Assign Data Security AI Admins membership only to the Copilot DLP policy owner. Document audit process and establish ownership for ongoing permission governance. | Provides comprehensive multi-workload permission audit with ongoing governance cadence and properly scoped DSPM roles for AI governance oversight. |
...
- **Purview Data Security AI Content Viewer** plus **Content Explorer Content Viewer** where current DSPM AI Interaction review requires both: Investigation team members authorized for prompt/response review
...
- **Separation of Duties:** Financial institutions should maintain separation of duties between front office (trading, sales), middle office (risk, compliance), and back office (operations, settlement). Permission audit should verify these boundaries are maintained across M365 workloads and are reflected in Copilot grounding tests. Data Security AI Admins membership and AI Content Viewer / Content Explorer Content Viewer access should not be held by the same person unless a documented exception is approved.
...
- **Client Data Segregation:** Wealth management and advisory firms should define firm-approved advisor-boundary rules for client data. Permission audit should verify that Copilot cannot surface one client's data to another client's advisor.
...
- **Terminated Employee Access:** Financial institutions should apply their approved access-revocation standard for terminated employees. Permission audit should verify that no stale access exists for former employees, as Copilot would inherit these permissions.
...
8. Purview Data Security AI Content Viewer role and Content Explorer Content Viewer access assigned to investigation team with documented authorization where AI Interaction prompt/response review is required (Recommended and Regulated levels)
...
- [Microsoft Learn: Permissions for Data Security Posture Management](https://learn.microsoft.com/en-us/purview/data-security-posture-management-permissions)
...
- [Microsoft Learn: Manage Teams settings](https://learn.microsoft.com/en-us/microsoftteams/manage-teams-overview)
...
- **Playbooks:** [Portal Walkthrough](../../playbooks/control-implementations/1.6/portal-walkthrough.md), [PowerShell Setup](../../playbooks/control-implementations/1.6/powershell-setup.md), [Verification & Testing](../../playbooks/control-implementations/1.6/verification-testing.md), [Troubleshooting](../../playbooks/control-implementations/1.6/troubleshooting.md)
