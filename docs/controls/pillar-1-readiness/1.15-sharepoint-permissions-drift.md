...
| Report count | Up to **five** site permissions for users reports; up to **two** organization-wide permissioned users reports (one per workload) |
...
Because of the 30-day re-run interval, DAG snapshot reports cannot supply the comparison baseline for monthly or weekly drift cadences on their own. Organizations targeting Recommended or Regulated cadences should treat DAG snapshots as a periodic reference point and use directly collected permission snapshots (Solution 17 `New-PermissionsBaseline.ps1`) plus DAG **recent activity** reports — which cover sharing link and "Everyone except external users" activity over a rolling 28-day window — as the higher-frequency drift signal.
