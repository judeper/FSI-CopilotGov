...
### User-Centric Drift Detection

In addition to site-level drift detection, organizations should monitor **per-user permission exposure drift** — changes in a user's aggregate access footprint over time:

| Drift Signal | Detection Method | Copilot Impact |
|-------------|-----------------|----------------|
| **User site access count increase** | Compare current per-user site access count against baseline (from DAG "Site permissions for users" report, Control 1.7) | Copilot's grounding scope for this user expanded — new sites may contain content the user should not access via AI |
| **Department boundary crossing** | User gains access to sites outside their department without a documented role change or project assignment | Cross-department access enables Copilot to synthesize information across organizational boundaries |
| **Group membership expansion** | User added to security groups that grant access to sensitive sites | Indirect permission grants may not be visible in site-level drift scans |
| **Sensitivity tier escalation** | User gains access to content with a higher sensitivity label than their previous baseline | Copilot can now surface Highly Confidential content that was previously outside this user's grounding scope |

**Integration with DSPM:** User-centric drift detection complements DSPM AI observability (Control 1.2) by correlating permission changes with Copilot usage patterns — a user who gains access to 50 new sites and immediately increases Copilot usage volume may warrant investigation.

**DAG snapshot report cadence constraints:** The Data Access Governance **Site permissions for users** and **Site permissions for your organization** snapshot reports carry Microsoft-documented generation limits that constrain how frequently they can serve as a drift comparison source:

| Constraint | Documented behaviour |
|-----------|---------------------|
| Data freshness | Reports capture data from up to **48 hours** before generation |
| Re-run interval | Each snapshot report can be generated again only **once every 30 days** |
| First-run latency | The first organization-wide permission state report takes up to **five days** to complete; subsequent reports complete within 24 hours |
| Report count | Up to **five** site permissions for users reports; the organization-wide report produces separate SharePoint and OneDrive results |
| Prerequisite | The site permissions for your organization report must be generated at least once before the per-user report can be created |

Because of the 30-day re-run interval, DAG snapshot reports cannot support drift cadences shorter than 30 days on their own. A monthly cadence sits at that boundary and must also account for up-to-48-hour data staleness and report-generation latency. Organizations targeting weekly or otherwise higher-frequency Recommended or Regulated cadences should treat DAG snapshots as a periodic reference point and use directly collected permission snapshots (Solution 17 `New-PermissionsBaseline.ps1`) plus DAG **recent activity** reports — which cover sharing link and "Everyone except external users" activity over a rolling 28-day window — as the higher-frequency drift signal.
...
- SharePoint Online Management Shell access (`Microsoft.Online.SharePoint.PowerShell` module version 16.0.25409 or later for Data Access Governance cmdlets)
- Microsoft Graph API permissions:
    - **Read / drift scanning:** `Sites.Read.All` (least-privileged permission for the Graph site permission read APIs; delegated or application)
    - **Auto-revert / write-back:** `Sites.FullControl.All`. The Graph `PATCH /sites/{site-id}/permissions/{permission-id}` and `DELETE /sites/{site-id}/permissions/{permission-id}` operations list `Sites.FullControl.All` as the only supported permission, and support **application** context only — delegated access is not supported. `Sites.ReadWrite.All` is not sufficient for these operations.
    - Graph `Update permission` cannot modify a **user** site permission; it applies to application permissions. Revert of user or group site permissions is performed through SharePoint Online Management Shell or PnP PowerShell rather than the Graph site permission API.
...
- [Microsoft Learn: Site permissions for users snapshot report](https://learn.microsoft.com/en-us/sharepoint/data-access-governance-site-permissions-users-report)
- [Microsoft Learn: Manage Data Access Governance reports with SharePoint Online PowerShell](https://learn.microsoft.com/en-us/sharepoint/powershell-for-data-access-governance)
- [Microsoft Learn: Graph API — update a site permission](https://learn.microsoft.com/en-us/graph/api/site-update-permission?view=graph-rest-1.0)
- [Microsoft Learn: Graph API — delete a site permission](https://learn.microsoft.com/en-us/graph/api/site-delete-permission?view=graph-rest-1.0)
