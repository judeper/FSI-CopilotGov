...
2. For Microsoft 365 Copilot and agents, confirm Microsoft Purview auditing is enabled before relying on interaction evidence. For third-party generative AI site signals, meet the capability-specific prerequisites described in [Microsoft's DSPM for AI prerequisites](https://learn.microsoft.com/en-us/purview/dspm-for-ai-considerations): device onboarding for sensitive-data visibility and endpoint DLP, an Edge configuration policy for Browser Data Security, and the Microsoft Purview browser extension on Windows users for an Insider Risk Management policy that detects site visits.
