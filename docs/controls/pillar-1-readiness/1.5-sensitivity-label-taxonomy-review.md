...
| **Copilot source-exclusion rules referencing labels** | The label taxonomy may not be mapped to supported Copilot-location rules | Labeled source content may be processed unless an applicable sensitivity-label exclusion rule is configured |
...
| **General / Internal** | Copilot can process; generated content can inherit "Internal" in supported creation experiences | Label inheritance or content-based auto-labeling on saved outputs; workload DLP for external sharing |
| **Confidential** | Copilot can process unless an applicable exclusion rule is configured; generated content can inherit "Confidential" where supported | Copilot-location DLP alerts on matching rules rather than logging every interaction; use inheritance or content-based auto-labeling for saved outputs |
| **Highly Confidential** | Copilot can process when the user has the required access and extraction rights unless an applicable exclusion rule is configured; generated content can inherit the label and protection where supported | Use documented Copilot exclusion rules plus workload DLP on saved/shared artifacts; encryption limits unauthorized output access |
