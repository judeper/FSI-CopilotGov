...
!!! info "Label Groups Transition"
    Microsoft is transitioning from parent/sublabel hierarchy to label groups (GA rollout began December 2025, completing May 2026). Organizations should plan for this transition when designing or updating their label taxonomy.

| Label Enhancement | Purpose | Implementation |
|-------------------|---------|----------------|
| **"Copilot Restricted" sublabel** | Restrict Copilot from processing content with this label | Apply encryption with restrictive usage rights via sensitivity label encryption settings. Organizations should verify Copilot-specific restriction behavior in their tenant, as support may vary by workload |
...
| **Auto-labeling for Copilot outputs** | Classify Copilot-generated documents | Auto-labeling policies using content-based conditions (SITs, trainable classifiers) applied to Copilot-generated content. Verify workload-specific support |
...
Inheritance Rules (behavior may vary by workload — verify per Copilot surface):
1. If single source: Output may inherit source label (or higher) depending on workload
2. If multiple sources: Output may inherit highest sensitivity label where supported
...
- [Microsoft Learn: Sensitivity labels and Microsoft 365 Copilot](https://learn.microsoft.com/en-us/purview/sensitivity-labels)
- [Microsoft Learn: DLP policies for Microsoft 365 Copilot](https://learn.microsoft.com/en-us/purview/dlp-learn-about-dlp)
