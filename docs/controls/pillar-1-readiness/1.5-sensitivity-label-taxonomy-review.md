...
| **Copilot Notebooks** | High | No standalone Notebook-level or container sensitivity label exists — Notebooks share a user-owned container with Copilot Pages and Loop My workspace, and automatic/recommended Notebook-level labeling is not a supported control (verify the current Pages-label support state in [Control 2.11](../pillar-2-security/2.11-copilot-pages-security.md)). This does **not** mean labeling stops applying to Notebook content: sensitivity labels already on referenced/source content, and any label applied to the shared Pages/Loop container itself, can still govern that content. Treat DLP (Microsoft 365 Copilot and Copilot Chat location) as the primary compensating control for Notebook-specific governance |
