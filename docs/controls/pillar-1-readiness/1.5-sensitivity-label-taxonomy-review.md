...
|-------------------|---------|----------------|
| **"Copilot Restricted" sublabel** | Mark content that should be excluded from Copilot grounding | Apply the label as a business classification, then enforce the exclusion with a DLP policy using the **Microsoft 365 Copilot and Copilot Chat** location (**Content contains > Sensitivity labels** → **Prevent Copilot from processing content**). For the requesting user, VIEW without EXTRACT normally prevents summarization but can still leave a link; it is not a universal Copilot block because OWNER includes EXTRACT and Microsoft documents data-in-use, Edge, and external-source exceptions. **This rule does not cover files a user uploads directly into a Copilot prompt** — see the limitation callout below |
...
| **Encryption settings** | Which labels apply encryption? What usage rights are configured? | Assess effective EXTRACT rights and the source/surface exceptions rather than treating encryption as a categorical Copilot block |
...
- If the label applies encryption, test the requesting user’s effective EXTRACT right (OWNER includes EXTRACT) and document data-in-use, Edge, and external-source exceptions rather than assuming the label template alone decides every Copilot outcome
