...
| **Copilot Notebooks** | High | No Notebook-level sensitivity label exists today — Microsoft's Loop/Copilot Pages sensitivity-label support does not extend to Copilot Notebooks (see [Control 2.11](../pillar-2-security/2.11-copilot-pages-security.md)). Govern Notebook content with compensating controls: DLP policies (Microsoft 365 Copilot and Copilot Chat location) and sensitivity labels on the source content a Notebook references, plus periodic re-verification as Microsoft expands label support |
