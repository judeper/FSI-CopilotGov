...
**License prerequisite:** Restricted Content Discovery can be applied to a SharePoint site only when **at least one user in the tenant is assigned a Microsoft 365 Copilot license** (a 2026 prerequisite confirmed on Microsoft Learn). This is distinct from the broader SAM licensing note above: while SAM administration is available through a Copilot license *or* the standalone SAM add-on, RCD specifically requires that the tenant hold at least one assigned Copilot license. Organizations should verify current licensing requirements at [Restrict discovery of SharePoint sites and content](https://learn.microsoft.com/en-us/sharepoint/restricted-content-discovery) before relying on RCD. *(Verified on Microsoft Learn, 2026-06-12.)*
