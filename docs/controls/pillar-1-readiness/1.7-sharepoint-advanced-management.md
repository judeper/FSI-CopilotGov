...
**Regulatory Reference:** GLBA 501(b), FFIEC IT Handbook (Information Security Booklet)
...
Evaluate and deploy SharePoint Advanced Management (SAM) capabilities that provide Copilot-specific governance features including Data Access Governance (DAG) reports, site access reviews, Restricted Content Discovery (RCD), and site lifecycle management. SAM extends the standard SharePoint admin experience with enterprise-grade governance tools that are critical for managing Copilot's interaction with SharePoint content at scale in regulated financial services environments.

---

## Why This Matters for FSI

- **GLBA 501(b):** SAM provides the technical tooling to implement and monitor safeguards for customer information stored in SharePoint. DAG reports specifically identify where customer data may be overshared, directly supporting GLBA safeguard requirements.
- **FFIEC IT Handbook (Information Security):** SAM features align with FFIEC expectations for access control monitoring, data access governance, and lifecycle management of information assets. These are foundational capabilities for technology risk management.
...
- **SEC Regulation S-P:** Restricted Content Discovery prevents Copilot from surfacing consumer financial information stored on specific SharePoint sites, supporting privacy safeguards.
- **Data Governance Best Practices:** SAM represents Microsoft's enterprise governance layer for SharePoint and is increasingly a prerequisite for effective Copilot governance at scale.
...
|------------|----------------------|--------------|
| **Data Access Governance (DAG) Reports** | Identify sites with oversharing, broad access, and sharing patterns that Copilot could exploit | SAM license required |
| **Site Access Reviews** | Trigger periodic access reviews with site owners to certify that current permissions are appropriate before Copilot deployment | SAM license required |
| **Restricted Content Discovery (RCD)** | Exclude specific SharePoint sites from Copilot content discovery while maintaining direct user access | SAM license required |
| **Site Lifecycle Management** | Automate inactive site detection, owner notification, and archival to remove stale content from Copilot's grounding scope | SAM license required |
| **Block Download Policy** | Prevent file downloads from specific sites, which also restricts Copilot's ability to process content from those sites | SAM license required |
| **Conditional Access for SharePoint Sites** | Apply site-level conditional access policies that restrict Copilot access in specific contexts | SAM license required |
| **Change History** | Track configuration changes to SharePoint sites for audit trail | SAM license required |
...
|---------|-------------|-------|
| Microsoft 365 E3 | No | SAM is an add-on |
| Microsoft 365 E5 | No | SAM is an add-on |
| SharePoint Advanced Management add-on | Yes | Per-user license, required for users managing governed sites |
| Microsoft 365 Copilot | No | Copilot license does not include SAM |
...
|----------------|-------------------------|-------------|
| **Microsoft 365 Copilot Chat** | Critical | RCD and DAG directly govern what Copilot Chat can access in SharePoint |
...
|-------|-------------|-----------|
| **Baseline** | Evaluate SAM licensing and capabilities for the organization. Generate DAG reports to understand current sharing and oversharing posture. Document SAM feature availability and gap analysis. | Minimum awareness of SAM capabilities and current data access posture, even if SAM is not yet licensed. |
| **Recommended** | Acquire SAM licensing. Enable DAG reporting with monthly review cadence. Configure RCD for sites containing highly sensitive data that should not be in Copilot scope. Initiate site access reviews for top 50 sites with broadest sharing. Enable site lifecycle management for inactive site detection. | Active use of SAM governance features to manage Copilot's SharePoint interaction at enterprise scale. |
| **Regulated** | All Recommended requirements plus: configure quarterly site access reviews for all sites containing regulated data. Enable RCD for all sites that have not passed data hygiene certification. Implement automated site lifecycle management with 90-day inactivity detection. Integrate DAG reports into compliance dashboards. Document SAM governance configuration in regulatory examination file. Establish SAM configuration change management process. | Comprehensive SAM governance that provides examination-ready data access controls and documented evidence of SharePoint governance for Copilot. |
...
Navigate to **Microsoft 365 Admin Center > Billing > Licenses** and verify SharePoint Advanced Management licenses are assigned to appropriate administrators.
...
4. Review the oversharing baseline report for aggregate risk scoring
5. Schedule recurring report generation
...
### Step 4: Initiate Site Access Reviews
...
### Step 5: Configure Site Lifecycle Management
...
- **Cost-Benefit of SAM Licensing:** SAM is an additional license cost. For regulated financial institutions deploying Copilot, the governance capabilities SAM provides (particularly DAG and RCD) are strongly recommended to support regulatory compliance obligations. Factor SAM licensing into Copilot deployment cost models.
...
- **M&A Data Room Governance:** Deal-related SharePoint sites should have RCD enabled by default, with DAG reporting used to monitor for permission drift during deal lifecycle.
...
1. SAM licensing status has been evaluated and documented (available, licensed, or gap identified)
2. DAG reports have been generated and reviewed within the past 30 days
3. DAG report findings have been triaged and assigned for remediation
4. RCD is enabled for sites containing highly sensitive data that should not be in Copilot scope (Recommended and Regulated levels)
5. Site access reviews have been initiated for sites with broadest sharing (Recommended and Regulated levels)
6. Site access review completion rates are tracked and reported
7. Site lifecycle management is configured with appropriate inactivity thresholds (Recommended and Regulated levels)
8. DAG reporting cadence is established (monthly minimum for Recommended; continuous for Regulated)
9. SAM configuration is documented in the organization's Copilot governance documentation
10. SAM governance features and their outputs are included in regulatory examination readiness materials (Regulated level)
...
- **Playbooks:** Playbook 1.7.1 (SAM Licensing and Enablement), Playbook 1.7.2 (DAG Report Configuration), Playbook 1.7.3 (RCD Configuration Walkthrough), Playbook 1.7.4 (Site Lifecycle Management Setup)
