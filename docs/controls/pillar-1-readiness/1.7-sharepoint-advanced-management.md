...
- **GLBA §501(b):** Legal/compliance should determine whether SAM evidence is needed to show safeguards for customer information stored in SharePoint. DAG reports and Restricted Access Control can provide visibility into who has access to customer information and enforce least-privilege access boundaries when the firm approves them for that purpose.
- **FFIEC IT Examination Handbook (Information Security):** Risk and technology owners should determine how SAM access-control monitoring, data-access governance, and lifecycle-management capabilities map to the firm's technology-risk program.
- **Sarbanes-Oxley §§302/404 (where applicable to ICFR):** ICFR owners should determine whether SAM site access reviews belong in periodic access certification evidence for sites containing financial data.
- **SEC Regulation S-P:** Privacy owners should determine whether Restricted Content Discovery or Restricted Access Control evidence is needed for SharePoint sites containing consumer financial information.
...
SharePoint Advanced Management (SAM) features that support Copilot deployment are available to SharePoint Admins when the tenant has at least one assigned Microsoft Copilot license. Organizations can also use the SharePoint Advanced Management add-on where Copilot licensing does not provide the needed administrative access.
...
| Microsoft Copilot | **Yes** | When at least one Microsoft Copilot license is assigned to a user, SharePoint Admins get access to the SAM features Microsoft lists for Copilot deployment support |
...
**Licensing note:** Verify licensing in the tenant before relying on a feature. Microsoft Learn lists the SAM features included with Copilot licenses, but features outside that list (for example, restricted site creation by apps) can still require the standalone SAM add-on. For SharePoint Admins who do not hold a Copilot license — such as IT staff managing SharePoint governance without using Copilot — confirm whether tenant-level Copilot licensing is sufficient for the admin experience or whether the standalone SAM add-on is required.

**Licensing history note:** If prior governance artifacts cite the Microsoft Ignite 2024 announcement or early 2025 rollout language for SAM with Microsoft 365 Copilot, treat those as historical context and validate the current entitlement in Microsoft Learn and in the tenant before relying on a feature.
...
|------------|----------------------|--------------|
| **Data Access Governance (DAG) Reports** | Identify sites with oversharing, broad access, and sharing patterns that Copilot could exploit across SharePoint and OneDrive | Included with SAM (Copilot license or add-on); E5-only access is limited to activity reports |
...
| **Restricted Access Control (RAC)** | Enforce a maximum access boundary on SharePoint and OneDrive sites, limiting access to specified Microsoft Entra security groups or Microsoft 365 groups regardless of prior permissions or sharing links | Included with SAM |
...
| **Content Management Assessment** | Identify inactive or ownerless sites, broken permission inheritance, EEEU exposure, and oversharing through sharing links that create Copilot oversharing risk | Included with SAM |
...
| **AI insights / SharePoint Admin Agent prompts** | Use AI insights on supported SAM reports and SharePoint Admin Agent prompts (for example, top agents accessing SharePoint and OneDrive content) to interpret governance risk and recommended actions; verify exact report availability in your tenant | Included with SAM where available |
...
### AI Insights and Agent Access Visibility

Microsoft Learn currently documents **AI insights** for SharePoint Advanced Management reports and the **SharePoint Admin Agent** prompt experience, including prompts such as "Show me a list of the top 10 agents that are accessing my SharePoint and OneDrive content." The M365 roadmap also lists M365 Agent Access Insights for SharePoint and OneDrive sites. Treat this as a tenant-verification item: report names, rollout state, and export options can vary by tenant and release stage.

| Aspect | Detail |
|--------|--------|
| **Portal** | SharePoint admin center > Reports > Data access governance or Change history > **Get AI insights** where available; SharePoint Admin Agent in Microsoft Copilot, SharePoint admin center, or Teams |
| **Coverage** | AI insights summarize supported reports; SharePoint Admin Agent can answer governance prompts over SharePoint and OneDrive content signals |
| **Time window** | Depends on the underlying report (for example, sharing-link activity reports cover the last 28 days; change history covers site actions and organization setting changes over the last 180 days) |
| **License requirement** | Requires Microsoft 365 Copilot license or standalone SAM license |
| **Data collection** | Depends on the underlying report; for DAG activity reports without full SAM, data collection must be enabled and is not retroactive |

**FSI relevance:** AI insights and SharePoint Admin Agent prompts can help institutions identify agent access patterns, oversharing, inactive sites, and governance gaps, but recommendations should be reviewed by accountable administrators before remediation.
...
- Natural-language admin queries are handled by the SharePoint Admin Agent rather than Catalog Management itself
...
**FSI relevance:** Run the Content Management Assessment before Copilot deployment to establish a baseline of content governance posture. Microsoft recommends using Content Management Assessment and DAG reports to identify high-risk sites and oversharing signals before broad Copilot and agentic experiences. Organizations should rerun CMA at least every 30 days during active remediation and compare results against previous baselines to track governance improvement. Also review Microsoft's secure-and-govern data foundation guidance for Copilot readiness: [Secure and govern data foundation for Microsoft 365 Copilot](https://learn.microsoft.com/en-us/microsoft-365/copilot/configure-secure-governed-data-foundation-microsoft-365-copilot).
...
**Configuration path:** SharePoint admin center > Sites > Active sites > [site] > Settings > turn **Restrict content from Microsoft Copilot** on.

**License prerequisite:** Restricted Content Discovery can be applied to a SharePoint site only when **at least one user in the tenant is assigned a Microsoft Copilot license** and SAM is available. This is distinct from the broader SAM licensing note above: while many SAM administration features are available through a Copilot license *or* the standalone SAM add-on, RCD is intended for Microsoft Copilot deployment and governance scenarios. Organizations should verify current licensing requirements at [Restrict discovery of SharePoint sites and content](https://learn.microsoft.com/en-us/sharepoint/restricted-content-discovery) before relying on RCD.

**Verification note:** A prior framework verification on 2026-06-12 used the same RCD Learn source; continue to use the current Learn page and tenant read-back as the source of truth before relying on the feature.
...
| Aspect | RAC Behavior |
|--------|-------------|
| **How it works** | Restricts access to SharePoint or OneDrive site content to users who both have site/content permission and are members of the configured control group |
| **Key distinction** | Unlike sharing permissions which grant additional access, RAC enforces a maximum access boundary -- anyone not in a configured control group cannot access the site even if they previously had permissions or a sharing link |
...
| **Scope** | Per-site configuration; up to 10 Microsoft Entra security groups or Microsoft 365 groups can be configured for a site; shared/private channel sites require separate configuration |
...
**Configuration path:** First enable site-level access restriction at **SharePoint admin center > Policies > Access control > Site-level access restriction**. Then configure a site at **SharePoint admin center > Sites > Active sites > [site] > Settings > Restricted site access**.
...
SAM's site lifecycle management policies help reduce Copilot's exposure to stale content. Treat thresholds such as 180 days for general inactive-site review or 90 days for heightened Regulated review as firm-approved policy choices, not Microsoft defaults.
...
| **Inactive detection** | Inactive site policies automatically detect inactive sites and notify site owners | Identifies stale content that may produce outdated Copilot responses |
...
- If the organization does not have Copilot licenses, verify whether the SharePoint Advanced Management add-on is provisioned and assigned to SharePoint Admins

### Step 2: Enable and Run DAG Reports

Navigate to **SharePoint admin center > Data access governance** and:

1. **Snapshot reports:** Run the "Site permissions across your organization" report (Recommended) for a point-in-time baseline of all site permissions. Run "Site permissions for users" to identify individual users with excessively broad access. Run "Sensitivity label applied to files" to identify unlabeled files.
2. **Activity reports:** Run the "Sharing links" report to identify sites with recent broad sharing activity. Run the "Shared with 'Everyone except external users'" report to identify EEEU sharing patterns.
3. Review snapshot report results to establish the pre-deployment permission baseline (retain as evidence).
4. Schedule recurring snapshot reports (quarterly minimum) and activity reports (monthly minimum).
...
Navigate to **SharePoint admin center > Site lifecycle management**:

1. Open the applicable policy area, such as **Inactive site policies**, **Site ownership policies**, or **Site attestation policies**
2. Configure owner or admin notification and enforcement actions
3. Define archival or remediation workflow for sites that remain inactive, ownerless, or unreviewed
4. Document retention/legal hold exceptions before archival or deletion actions

---

## Financial Sector Considerations

- **SAM Licensing Clarification:** Organizations with at least one assigned Microsoft Copilot license get the SAM features Microsoft lists for Copilot deployment support. SharePoint Admins should still verify their tenant experience, admin role assignment, and whether any desired feature requires the standalone SAM add-on.
...
- **RAC for MNPI and NPI Sites:** Sites containing material non-public information (M&A deal rooms, pre-announcement financials) or non-public personal information (customer account data, credit files) should have RAC enabled to enforce hard access boundaries. RAC is particularly effective for ensuring that sharing links do not bypass intended access restrictions where the firm maps the site to GLBA §501(b), SEC Regulation S-P, or internal privacy requirements.
- **Access Review Regulatory Alignment:** SAM site access reviews can serve dual purpose for Sarbanes-Oxley access certification evidence where ICFR owners determine that the SharePoint site is in scope. Coordinate SAM access reviews with existing SOX compliance calendars to avoid duplicative effort.
...
- **Site Lifecycle for Regulatory Retention:** Site lifecycle management automation must respect the firm's approved retention and legal-hold requirements. Configure archival and deletion policies to align with FINRA 4511, SEC 17a-4, and institution-specific retention schedules where records owners determine those sources apply.
...
2. DAG snapshot and activity reports have been generated and reviewed within the past 30 days
3. "Site permissions across your organization" snapshot report has been generated as a pre-deployment baseline (documented with date)
...
- [Microsoft Learn: SharePoint Advanced Management licensing](https://learn.microsoft.com/en-us/sharepoint/sharepoint-advanced-management-features-copilot-license)
...
- **Related Controls:** [1.2 SharePoint Oversharing Detection](1.2-sharepoint-oversharing-detection.md), [1.3 Restricted SharePoint Search](1.3-restricted-sharepoint-search.md), [1.6 Permission Model Audit](1.6-permission-model-audit.md), [3.1 Copilot Audit Logging](../pillar-3-compliance/3.1-copilot-audit-logging.md), [4.5 Usage Analytics](../pillar-4-operations/4.5-usage-analytics.md)
- **Playbooks:** [Portal Walkthrough](../../playbooks/control-implementations/1.7/portal-walkthrough.md), [PowerShell Setup](../../playbooks/control-implementations/1.7/powershell-setup.md), [Verification & Testing](../../playbooks/control-implementations/1.7/verification-testing.md), [Troubleshooting](../../playbooks/control-implementations/1.7/troubleshooting.md)
