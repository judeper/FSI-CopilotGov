...
> **Note — Anthropic (Microsoft subprocessor) and SpaceXAI/xAI (independent provider).**
>
> - **Anthropic** has been a Microsoft subprocessor for Microsoft 365 Copilot since **January 7, 2026**. Anthropic models provided under the subprocessor arrangement are covered by the Microsoft Product Terms and Microsoft Data Protection Addendum (DPA), and the Microsoft Customer Copyright Commitment applies to Anthropic models used within products covered by that commitment (including Microsoft 365 Copilot and Copilot Studio). Per Microsoft's published guidance, Anthropic models are **out of scope for the EU Data Boundary and, when applicable, in-country processing commitments**. Anthropic models are **on by default for most commercial-cloud customers**. They are **not** on by default in the European Union, EFTA, or the United Kingdom: in those regions the setting appears but defaults to **No users**. Availability has continued to expand since initial onboarding: Microsoft introduced a **Copilot in Microsoft 365 apps with Anthropic models** setting for EU/EFTA and UK on **April 3, 2026**. Financial institutions should include Anthropic in their third-party risk assessment scope and confirm whether their tenant has Anthropic enabled by default.
> - **Anthropic models with Data Retention** are a separate category that sits **outside** the subprocessor arrangement. For these models Anthropic acts as an **independent data processor rather than a Microsoft subprocessor**, the Microsoft Product Terms and DPA **do not apply**, and use is governed by Anthropic's separate Commercial Terms of Service and Data Protection Addendum. They are **default-off in all scenarios**, including regions where other Anthropic models are on by default, and require explicit tenant admin opt-in plus user or group assignment. Anthropic stores most inputs and outputs for up to 30 days; where its trust and safety classifiers identify potential usage-policy violations, Anthropic may retain content for up to two years and classification scores for up to seven years. Anthropic states it does not use retained data for model training without express permission. Treat these models as a discrete fourth-party data-flow path within third-party risk assessment scope.
> - **SpaceXAI (xAI)** models are hosted outside Microsoft-managed environments and audit controls. Microsoft's customer agreements (including the Product Terms and DPA), data residency commitments, audit and compliance requirements, SLAs, and the Customer Copyright Commitment **do not apply** to use of SpaceXAI services; use is governed by xAI's separate Enterprise Terms of Service and Data Processing Addendum. SpaceXAI is opt-in only and currently scoped to Copilot Studio. Treat it as a discrete additional vendor that requires its own due diligence (security review, contractual review, data-flow analysis, and access scoping) under OCC Bulletin 2023-17 and GLBA §501(b).
>
> Review Microsoft's published subprocessor list (Service Trust Portal) and the AI provider settings under M365 Admin Center > Copilot > Settings > View all to confirm which providers are currently available to the tenant. Anthropic appears under **AI providers operating as Microsoft subprocessors**; SpaceXAI appears under **AI providers for other large language models**. Changing the Anthropic setting requires the **AI Administrator** or **Global Administrator** role; changing the SpaceXAI setting requires the **Global Administrator** role. Provider terms, default states, role requirements, and portal paths were reverified against Microsoft Learn on **2026-09-23**.
...
| **Data boundary** | EU Data Boundary and contractual data residency commitments apply to Microsoft-hosted Copilot processing; **Anthropic models (Microsoft subprocessor since Jan 7, 2026) are out of scope for the EU Data Boundary and in-country processing commitments**; **Anthropic models with Data Retention fall outside the Microsoft Product Terms and DPA entirely (Anthropic acts as an independent data processor)**; **SpaceXAI models are processed outside Microsoft-managed environments entirely and are not covered by Microsoft's data residency commitments** | Review data residency documentation; confirm provider-specific exclusions in Microsoft 365 Copilot privacy, "Anthropic models in Microsoft Online Services", and "Connect to SpaceXAI models" Learn pages |
...
| **Regulated** | All Recommended requirements plus: engage legal counsel to review Microsoft contractual terms for AI service provisions, the separate xAI Enterprise Terms of Service / Data Processing Addendum where SpaceXAI is enabled, and Anthropic's separate Commercial Terms of Service / Data Protection Addendum where Anthropic models with Data Retention are enabled. Conduct fourth-party risk assessment for AI subprocessors (including Anthropic, with explicit documentation of the EU Data Boundary / in-country processing exclusion) and a discrete vendor risk assessment for provider paths outside the Microsoft DPA (SpaceXAI; Anthropic models with Data Retention). Evaluate Microsoft's AI incident response capabilities. Assess concentration risk and develop contingency plans. Include AI vendor risk in board risk committee reporting. Conduct annual vendor risk re-assessment. Maintain vendor risk documentation in regulatory examination file. | Comprehensive AI vendor risk management that meets interagency guidance expectations with legal review, fourth-party assessment, board reporting, and examination readiness, including provider-specific data-boundary exceptions. |
...
- [Connect to SpaceXAI models (independent processor)](https://learn.microsoft.com/en-us/microsoft-365/copilot/connect-to-ai-models)
