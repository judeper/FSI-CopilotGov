...
**Regulatory Reference:** OCC Bulletin 2023-17, Interagency Guidance on Third-Party Relationships (2023), FFIEC IT Handbook (Outsourcing Technology Services), GLBA §501(b)
...
- **OCC Bulletin 2023-17 (Third-Party Relationships):** Requires national banks and federal savings associations to practice effective risk management for all third-party relationships, with heightened expectations for critical activities. Microsoft 365 Copilot -- which processes institutional data through AI models -- qualifies as a critical third-party activity requiring comprehensive due diligence.
...
> **Note — Anthropic (Microsoft subprocessor) and xAI (independent provider).**
>
> - **Anthropic** has been a Microsoft subprocessor for Microsoft 365 Copilot since **January 7, 2026**. Anthropic models provided under the subprocessor arrangement are covered by the Microsoft Product Terms and Microsoft Data Protection Addendum (DPA), and the Microsoft Customer Copyright Commitment applies to Anthropic models used within products covered by that commitment (including Microsoft 365 Copilot and Copilot Studio). Per Microsoft's published guidance, Anthropic models are **out of scope for the EU Data Boundary and, when applicable, in-country processing commitments**. Anthropic models are **on by default for most commercial-cloud customers (excluding EU/EFTA and UK)**, where the setting defaults to **No users**. Availability has continued to expand since initial onboarding: Microsoft introduced a **Copilot in Microsoft 365 apps with Anthropic models** setting for EU/EFTA and UK on **April 3, 2026**, and on **July 22, 2026** added a setting allowing **non-federal Government Community Cloud (GCC)** customers to use Anthropic models (disabled by default; when enabled, Customer Data is processed outside Microsoft's FedRAMP-authorized US Government cloud). Anthropic models are **not available** to federal GCC customers, or to any customers in GCC High, DoD, or other sovereign clouds. Financial institutions should include Anthropic in their third-party risk assessment scope and confirm whether their tenant has Anthropic enabled by default.
> - **Anthropic "Preview models with Data Retention"** are a separate category that sits **outside** the subprocessor arrangement. For these models Anthropic acts as an **independent data processor rather than a Microsoft subprocessor**, the Microsoft Product Terms and DPA **do not apply**, and use is governed by Anthropic's separate Commercial Terms of Service and Data Protection Addendum. They are **default-off in all scenarios**, including regions where other Anthropic models are on by default, and require explicit tenant admin opt-in plus user or group assignment. Anthropic stores most inputs and outputs for up to 30 days; where its trust and safety classifiers identify potential usage-policy violations, Anthropic may retain content for up to two years and classification scores for up to seven years. Anthropic states it does not use retained data for model training without express permission. Treat these models as a discrete fourth-party data-flow path within third-party risk assessment scope.
> - **xAI** models are hosted by xAI **outside Microsoft-managed environments and audit controls**. Microsoft's customer agreements (including the Product Terms and DPA), data residency commitments, audit and compliance requirements, SLAs, and the Customer Copyright Commitment **do not apply** to use of xAI services; use is governed by xAI's separate Terms of Service and Data Processing Addendum. xAI is opt-in only and currently scoped to Copilot Studio. Treat xAI as a discrete additional vendor that requires its own due diligence (subprocessor mapping, security review, contractual flow-down, data flow analysis) under OCC Bulletin 2023-17 and GLBA §501(b).
>
> Review Microsoft's published subprocessor list (Service Trust Portal) and the AI provider settings under M365 Admin Center > Copilot > Settings > View all to confirm which providers are currently available to the tenant. Anthropic appears under **AI providers operating as Microsoft subprocessors**; xAI appears under **AI providers for other large language models**. Changing the Anthropic setting requires the Entra AI Administrator or Entra Global Admin role; changing the xAI setting requires the Entra Global Admin role.
...
| **Data boundary** | EU Data Boundary and contractual data residency commitments apply to Microsoft-hosted Copilot processing; **Anthropic models (Microsoft subprocessor since Jan 7, 2026) are out of scope for the EU Data Boundary and in-country processing commitments**; **Anthropic Preview models with Data Retention fall outside the Microsoft Product Terms and DPA entirely (Anthropic acts as an independent data processor)**; **xAI models are processed outside Microsoft-managed environments entirely and are not covered by Microsoft's data residency commitments** | Review data residency documentation; confirm provider-specific exclusions in Microsoft 365 Copilot privacy, "Anthropic models in Microsoft Online Services", and "Connect to xAI's models" Learn pages |
...
| **Copilot Notebooks** | High | AI-generated collaborative content |
...
| **Microsoft Scout (Frontier preview)** | Critical | Scout introduces a **third-party inference boundary** — some inference is processed through GitHub Copilot and third-party model providers under those providers' separate terms, outside the Microsoft 365 DPA. Session and memory data remain in OneDrive under M365 tenant controls, but automation instructions and MCP-server output stored locally on the endpoint are also outside the M365 DPA. Reflect the GitHub Copilot entitlement path, the third-party inference boundary, and the local artifact boundary in vendor-risk artifacts and in Control [4.16](../pillar-4-operations/4.16-microsoft-scout-governance.md) governance decisions. |
...
| **Regulated** | All Recommended requirements plus: engage legal counsel to review Microsoft contractual terms for AI service provisions, the separate xAI Terms of Service / Data Processing Addendum where xAI is enabled, and Anthropic's separate Commercial Terms of Service / Data Protection Addendum where Anthropic Preview models with Data Retention are enabled. Conduct fourth-party risk assessment for AI subprocessors (including Anthropic, with explicit documentation of the EU Data Boundary / in-country processing exclusion) and a discrete vendor risk assessment for any provider paths that fall outside the Microsoft DPA (xAI; Anthropic Preview models with Data Retention). Evaluate Microsoft's AI incident response capabilities. Assess concentration risk and develop contingency plans. Include AI vendor risk in board risk committee reporting. Conduct annual vendor risk re-assessment. Maintain vendor risk documentation in regulatory examination file. | Comprehensive AI vendor risk management that meets interagency guidance expectations with legal review, fourth-party assessment, board reporting, and examination readiness, including provider-specific data-boundary exceptions. |
...
- Subscribe to Microsoft's subprocessor change notifications. Per the Microsoft DPA, Microsoft commits to at least six months' advance notice before adding a subprocessor that processes Customer Data, except for subprocessors supporting AI functionality, where Microsoft provides at least 30 days' notice and customers can disable use of that subprocessor for at least six months after notice. Notice is given by updating the Microsoft Online Services Subprocessor List on the Service Trust Portal.
...
- **Board Oversight:** OCC Bulletin 2023-17 requires board oversight of critical third-party relationships. If Copilot is deployed at enterprise scale, the AI vendor relationship warrants board risk committee reporting, at minimum annually.
...
- [Microsoft Online Services Subprocessor List (Service Trust Portal)](https://aka.ms/subprocessor)
- [Microsoft Supplier Management and Subprocessor Categories](https://learn.microsoft.com/en-us/compliance/assurance/assurance-supplier-management)
...
- [Connect to xAI's models](https://learn.microsoft.com/en-us/microsoft-365/copilot/connect-to-ai-models)
