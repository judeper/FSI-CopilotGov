...
RSS supports a **hard maximum of 100 SharePoint sites** on the allow-list — this is an enforced cap, not advisory guidance, and Microsoft positions RSS as a short-term measure that does not scale as Copilot and agentic operations grow. The 100-site limit **includes Hub sites but not their associated (sub) sites**; enabling a Hub site does not automatically extend the allow-list to its associated sites. This creates a governance decision:
