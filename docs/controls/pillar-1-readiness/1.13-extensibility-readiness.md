...
**Regulatory Reference:** GLBA §501(b), FFIEC IT Handbook (Information Security Booklet), OCC Bulletin 2023-17 (Third-Party Relationships)
...
- **AI governance source monitoring:** Firms should define the primary sources, internal policies, and supervisory expectations they use for AI systems that interact with external data sources or take actions based on AI processing, including evaluation of data quality, bias, and operational risk where approved.
...
| **Management rules** | Has the organization evaluated Agent management rules in the Microsoft 365 admin center for bulk governance actions? | The supported actions are **Install Microsoft agents**, **Reassign ownerless Agent Builder agents to manager**, **Block ownerless agents without usage**, **Apply a template to agents**, and **Reject publish requests older than a specified number of days**. Publisher restriction remains a separate **Allowed agent types** setting. The **Apply template** action is a one-time bulk operation, not scheduled enforcement |
...
| **Policy templates** | Has the organization evaluated policy templates for consistent policy application? | Policy templates remain relevant when establishing security policies for new agents. Separately, the **Apply template** management-rule action applies a preexisting or custom template to selected existing published agents at scale. That one-time action evaluates Registry agents that have an agent identity; it does not apply to agent blueprints or AI teammates |
...
- **Agents > Settings > Agent management rules** — select from the five supported bulk actions; for **Apply template**, define criteria, select a preexisting or custom template, review all eligible agents or a subset, and run the one-time operation
...
- **Agents > Settings > Security templates** — configure predefined policies for new agents; do not conflate this with the **Apply template** management-rule action for eligible existing published agents
