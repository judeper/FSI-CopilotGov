...
- Maintain a current inventory of all Copilot licenses (type, quantity, assignment), including any PAYG-enabled user groups — licensed Copilot is $30/user/month (full M365 Copilot); Copilot Chat PAYG is $0.01/message for work data queries via Azure Commerce. Track licenses by department:
...
- Track total Copilot licensing cost on a monthly basis, including both per-seat and PAYG metered costs — **Portal:** M365 Admin Center > Reports > Microsoft 365 Copilot usage; M365 Admin Center > Billing > Pay-as-you-go services
...
- Configure PAYG billing link if applicable — **Portal:** M365 Admin Center > Billing > Your products > PAYG setup (link to Azure subscription)

### Recommended

- Implement usage-based license optimization with 60-day inactivity threshold
- Enable PAYG billing for occasional users with documented approval workflow; configure budget alert caps per department before enabling PAYG access — **Portal:** Azure portal (`portal.azure.com`) > Cost Management + Billing > Budgets > + Add. **Limitation:** No native hard-stop budget cap exists for PAYG in M365; Azure Cost Management provides notification-based budget alerts only
- Create department-level cost allocation reports for internal chargeback or showback, including separation of per-seat and PAYG costs — tag PAYG costs using Azure resource tags for department chargebacks: Azure portal > Cost Management > Cost analysis > Filter by "Microsoft 365"
- Configure Azure Cost Management tags for PAYG cost routing to correct cost centers; verify tag compliance monthly
- Establish ROI tracking that compares license costs to measured productivity benefits; include PAYG cost-per-interaction in ROI calculations — use per-user detail reports for granularity: `GET /reports/getMicrosoft365CopilotUserDetailReport(period='D30')`. **Limitation:** Graph API has limited per-user granularity for Copilot Chat PAYG usage
...
- Conduct quarterly license optimization reviews with department stakeholders, including per-seat vs. PAYG breakeven analysis (breakeven at approximately 3,000 messages/user/month)
...
- For PAYG deployments: configure Azure Cost Management budget alerts per department and document the authorization hierarchy for budget cap adjustments — **Limitation:** No native hard-stop budget cap in M365 Admin Center for PAYG Copilot Chat; Azure alerting is notification-only. Document compensating controls for SOX ITGC procedures
...
- Conduct quarterly PAYG cost anomaly reviews to detect unusual usage spikes or potential misuse — use Azure Cost Management > Cost analysis filtered by M365 service
- Conduct annual independent review of license optimization effectiveness
- Document license allocation methodology for auditor review, including PAYG cost allocation tag governance — export license assignments by department using `Get-MgUser` filtered by `SkuPartNumber "MICROSOFT_365_COPILOT"`
