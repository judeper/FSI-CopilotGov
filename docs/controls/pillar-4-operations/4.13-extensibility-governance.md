...
> **Agent identity and control-plane governance are authoritative in FSI-AgentGov.** This control governs the **Copilot-side operational monitoring** of extensibility and published agents through the M365 admin center (Integrated apps, Agents > Overview / Registry / Settings). Agent *identity* governance (Entra Agent ID, Conditional Access for agent identities) and the Agent 365 *control-plane* configuration are specified in **[FSI-AgentGov Control 2.26 — Entra Agent ID Identity Governance](https://judeper.github.io/FSI-AgentGov/controls/pillar-2-management/2.26-entra-agent-id-identity-governance/)** and **[FSI-AgentGov Control 2.25 — Agent 365 Admin Center Governance Console](https://judeper.github.io/FSI-AgentGov/controls/pillar-2-management/2.25-agent-365-admin-center-governance-console/)**. Consume and cross-reference those controls; Power Platform Inventory (referenced below) is likewise an FSI-AgentGov operational surface.
...
| **Agent tools** | Agents > Tools | AI-powered tools, MCP servers, availability/blocking, and registration requests |
...
| **Agents at risk** | Aggregates high-severity risk signals from Microsoft security platforms and connected third-party platforms |

Microsoft Learn notes that some Agent Overview metrics begin when Agent 365 licenses are activated and can initially reflect fewer than 30 days of data; treat metric completeness as tenant evidence rather than assuming a full baseline on day one.
...
| Agent tools and BYO MCP servers | High | Microsoft Learn describes **Agents > Tools** as the centralized view for AI-powered tools and MCP servers; BYO MCP server registration is preview and requires admin review, approval/rejection, and Entra permission consent before supported clients can use the server |
| Microsoft Scout (Frontier preview) — skills, subagents, MCP | High | Scout composes multi-step work through skills, delegated subagents, and MCP-server tools. Treat these as extensibility surfaces under this control; MCP-server approvals are shared with [Control 2.16](../pillar-2-security/2.16-federated-connector-mcp-governance.md), and Scout-specific approval criteria are in [Control 4.16](4.16-microsoft-scout-governance.md). |
...
4. Review **Agents > Tools** for available/blocked tools, MCP servers, and pending requests; for BYO MCP servers, capture declared tools and Entra consent state.
5. Consolidate the data into one governance inventory.
...
2. Confirm allowed types, sharing, templates, user access, feedback sharing, and tags align with policy.
...
**Work IQ and write operations:** When Work IQ MCP is used in Copilot Studio or agent-building surfaces, Microsoft Learn states it is preview, uses Copilot Credits / consumptive billing, is read-only unless an administrator explicitly enables write operations, and may have region-dependent allow/block controls. Include Work IQ enablement, spending policy, and write-operation state in the operational review rather than assuming read-only behavior from public documentation alone.

## Advisory: Multi-Agent Orchestration and Agent-to-Agent (A2A) Security

!!! warning "GA — April 2026"

    Multi-agent orchestration is now generally available for supported Copilot Studio patterns. Copilot Studio agents can orchestrate with other agents — including Microsoft Fabric agents — via agent-to-agent (A2A) protocols. Agents can call other agents as tools, creating chained execution flows.

**External A2A endpoints (July 2026 Copilot Studio documentation):** The `Connect to an agent that supports the A2A protocol` authoring surface now documents A2A connections to arbitrary external agents — including agents built on non-Microsoft frameworks, hosted outside Copilot Studio, or running on-premises or in another cloud through custom-connector infrastructure. When an external A2A endpoint is invoked, Copilot Studio transmits structured A2A metadata that includes a `contextId`, message IDs, locale, and the **full chat history** (not only the latest user utterance). Authentication choices are limited to **None**, **API key**, or **OAuth 2.0**. Treat each external A2A endpoint as a third-party data processor for the purpose of vendor risk, data residency, and DLP evaluation — the deep authoring/publishing lifecycle for external A2A agents is out of scope of Microsoft 365 Copilot governance and belongs in **[FSI-AgentGov Control 2.17 — Multi-Agent Orchestration Limits](https://judeper.github.io/FSI-AgentGov/controls/pillar-2-management/2.17-multi-agent-orchestration-limits/)**.
...
- **A2A payload disclosure:** The A2A metadata payload includes the full prior chat history for the delegating agent. Before approving an external A2A endpoint, review whether that transcript may contain regulated data (MNPI, customer NPI, records subject to SEC Rule 17a-4 for required broker-dealer records) and reflect that in the DLP and residency assessment for the endpoint.
...
- **Third-party agent risk:** If an orchestration chain includes third-party or partner-built agents (including any external A2A endpoint), the data governance and residency implications extend to the third party's execution environment. This should be reflected in vendor risk assessments ([Control 1.10](../pillar-1-readiness/1.10-vendor-risk-management.md)); cross-tenant A2A patterns are additionally in scope of [Control 2.17](../pillar-2-security/2.17-cross-tenant-agent-federation.md).

**Recommended actions:**

1. Inventory all agents that participate in multi-agent orchestration chains, distinguishing internal Microsoft agents (Copilot Studio, Fabric) from external A2A endpoints
2. Map data flow paths across agent-to-agent calls and identify DLP boundary gaps; for external A2A endpoints, record the endpoint URL, hosting jurisdiction, and authentication mode (None / API key / OAuth 2.0)
...
5. Update agent approval workflows to require A2A orchestration disclosure, including a business justification for any external A2A endpoint and confirmation that the endpoint's data-handling posture has been reviewed under [Control 1.10](../pillar-1-readiness/1.10-vendor-risk-management.md)
...
| 7 | Review tools and MCP governance | Agent tools, MCP servers, BYO MCP requests, approval/rejection decisions, and Entra consent state are documented |
| 8 | Confirm Work IQ write posture | Work IQ usage-based billing, spending policy, allow/block availability, and write-operation enablement are verified in the tenant |
...
- [Manage tools for agents in Microsoft 365 admin center](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/manage-tools-for-agent?view=o365-worldwide)
- [Work IQ in Copilot Studio (preview)](https://learn.microsoft.com/en-us/microsoft-copilot-studio/use-work-iq)
