...
The Copilot Dashboard provides **intelligent summaries** — AI-generated narrative insights that synthesize adoption and impact trends — and **industry benchmarks** that compare the organization's Copilot impact metrics against anonymized peer data. These features require E5 or Viva Insights licensing and are not available in GCC High or DoD environments (see [Control 4.5 — Copilot Dashboard Feature Availability Matrix](4.5-usage-analytics.md#copilot-dashboard-feature-availability-matrix) for the full availability matrix).
...
Viva Insights impact data measures behavioral signals (assisted work patterns, time saved), but does not directly capture whether employees perceive Copilot as useful. Integrating employee sentiment data from **Viva Pulse** or **Viva Glint** adds qualitative context to quantitative impact metrics:
