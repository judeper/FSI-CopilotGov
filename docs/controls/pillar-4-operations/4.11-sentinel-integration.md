...
**Regulatory Reference:** FFIEC IT Examination Handbook (Information Security), Sarbanes-Oxley §§302/404, NYDFS Part 500, GLBA §501(b)
...
!!! warning "Sentinel Azure Portal Retirement — March 31, 2027"
    After **March 31, 2027**, Microsoft Sentinel will no longer be supported in the Azure portal. Sentinel will be available exclusively in the **Microsoft Defender portal**. All customers currently using Microsoft Sentinel in the Azure portal will be redirected to the Defender portal. FSI teams should begin planning their transition now to avoid disruption to monitoring and detection workflows. See [Microsoft Sentinel in the Microsoft Defender portal](https://learn.microsoft.com/en-us/azure/sentinel/microsoft-sentinel-defender-portal) and the [transition guide](https://learn.microsoft.com/en-us/azure/sentinel/move-to-defender) for migration steps. Organizations should verify that all Sentinel workbooks, analytics rules, SOAR playbooks, and data connectors described in this control are accessible and operational in the Defender portal before the retirement date.
...
| Microsoft Copilot (Preview) | M365 Copilot interaction events in the **CopilotActivity** table (start with `RecordType == "CopilotInteraction"` and validate any additional RecordType values in-tenant) | Enable the dedicated **"Microsoft Copilot"** data connector (public preview) in Sentinel; see note on line 16 |
...
The CopilotActivity table captures normalized Copilot audit records. Start with these documented `RecordType` values and validate any additional tenant-specific values in your own logs:

| Event Type | Description | KQL Table |
|-----------|-------------|-----------|
| CopilotInteraction | User interaction with Microsoft-developed Copilot apps | **CopilotActivity** |
| ConnectedAIAppInteraction | Interaction with connected/custom AI apps deployed in-tenant | **CopilotActivity** (verify in tenant) |
| AIAppInteraction | Interaction with third-party AI apps not deployed in-tenant | **CopilotActivity** (verify in tenant) |
...
!!! warning "Requires Microsoft Copilot Connector (Preview)"
...
!!! info "Assessment collector data minimization"
    The `Collect-Sentinel.ps1` evidence payload stores aggregate/non-PII metadata only (`HasRecords`, `RecordCount`, status, schema/query metadata). It does **not** persist raw CopilotActivity rows containing `ActorUserId`, `ActorName`, `SrcIpAddr`, or `LLMEventData`.
...
| summarize SensitiveAccessCount = count() by ActorUserId, ActorName, SensitivityLabelId
| where SensitiveAccessCount > 50
| sort by SensitiveAccessCount desc
```
...
| where TimeGenerated > ago(7d)
| where RecordType == "CopilotInteraction"
| where tostring(LLMEventData) has "InformationBarrier"
| project TimeGenerated, ActorUserId, ActorName, SrcIpAddr, Workload, LLMEventData
| sort by TimeGenerated desc
```
...
| where RecordType == "CopilotInteraction"
| summarize AvgDaily = count() / 30.0 by ActorUserId;
CopilotActivity
| where TimeGenerated > ago(1d)
| where RecordType == "CopilotInteraction"
| summarize TodayCount = count() by ActorUserId
| join kind=inner baseline on ActorUserId
| where TodayCount > AvgDaily * 3
| project ActorUserId, TodayCount, AvgDaily, Ratio = TodayCount / AvgDaily
```

**Query 6: Agent 365 and Copilot Studio Agent Activity**

!!! warning "Requires Microsoft Copilot Connector (Preview)"
    Agent 365 and Copilot Studio invocation events are Copilot-interaction records delivered to the **`CopilotActivity`** table via the dedicated "Microsoft Copilot" data connector. If that connector is not enabled, Sentinel queries should fail closed (`CopilotActivity` unavailable or zero records) and should not be replaced with `OfficeActivity`.

```kql
CopilotActivity
| where TimeGenerated > ago(24h)
| where RecordType == "CopilotInteraction"
| where tostring(LLMEventData) has_any ("Agent", "agent", "Copilot Studio")
| summarize SessionCount = count(), DistinctSourceIps = dcount(SrcIpAddr)
    by ActorUserId, ActorName, Workload
...
| where TimeGenerated > ago(7d)
| where RecordType == "CopilotInteraction"
| where tostring(LLMEventData) has_any ("scheduled", "automated", "flow")
| summarize AutomatedCount = count() by ActorUserId, ActorName, bin(TimeGenerated, 1d)
...
!!! info "Customize Third-Party AI Detection"
...
> **Note:** Security Copilot (Microsoft Defender / Sentinel AI agent) is a distinct product from M365 Copilot. Its events are NOT ingested into the `CopilotActivity` table. Verify with your Defender XDR connector which table captures Security Copilot autonomous agent activity in your tenant. The query below uses `OfficeActivity` as a starting point but may return zero rows pending Microsoft's connector documentation for Security Copilot events.
...
1. Enable **Microsoft 365** connector:
   ...
   - Note: this connector populates the `OfficeActivity` table for Exchange, SharePoint, and Teams events; it does **not** deliver CopilotInteraction records
2. Enable the **Microsoft Copilot** data connector (public preview):
   - This populates the `CopilotActivity` table with M365 Copilot interaction events
   - Required for KQL queries targeting `CopilotActivity` records (starting with `RecordType == "CopilotInteraction"`)
3. Enable **Microsoft Purview Information Protection** connector (if available)
4. Enable **Microsoft Entra ID** connector:
   - Select sign-in logs and audit logs
5. Enable **Microsoft Defender for Cloud Apps** connector
6. Verify data ingestion:
   ...
   | count
   ```
   > If the count is zero, confirm the Microsoft Copilot (Preview) connector is enabled and configured. The `OfficeActivity` table will not contain CopilotInteraction records regardless of the M365 connector state.
...
2. Configure rule settings:
   ...
   - **Entity mapping:** Map ActorUserId, SrcIpAddr, and RecordId entities for investigation context
...
- **SCU consumption alerts:** Organizations may want to create Sentinel workbook panels or alert rules to track Security Copilot SCU consumption alongside other Copilot usage metrics. Do not assume that inclusion-model throttling or $6/SCU pay-as-you-go is already active; Microsoft describes both as future availability after notice.

**Recommended actions:**

1. Verify Sentinel data connectors capture Security Copilot activity events
2. Create or import analytics rules for Security Copilot agent-specific detections
3. Add Security Copilot panels to existing Copilot security workbooks
...
- [Transition Microsoft Sentinel to the Defender portal](https://learn.microsoft.com/en-us/azure/sentinel/move-to-defender)
- [Office 365 connector for Sentinel](https://learn.microsoft.com/en-us/azure/sentinel/data-connectors-reference#microsoft-365-formerly-office-365)
...
- [Microsoft Sentinel SOAR Capabilities](https://learn.microsoft.com/en-us/azure/sentinel/automation)
