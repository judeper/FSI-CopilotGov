...
!!! abstract "Consume-and-cross-link control"
    ...
    - **[FSI-AgentGov Control 2.25 — Agent 365 Admin Center Governance Console](https://judeper.github.io/FSI-AgentGov/controls/pillar-2-management/2.25-agent-365-admin-center-governance-console/)** — Agent registry and ownership visibility.
        ...
        For roadmap monitoring, use the **Microsoft AI at Work Roadmap** rather than historical Power Platform or Copilot Studio release-plan pages for new capabilities. Existing release plans are historical reference only after Microsoft's September 2026 release-plan migration.
...
- [ ] Treat **AI model selection** for agents as a change event recorded in the FSI-AgentGov lifecycle record. Record the effective model rather than only the moving **Default** alias; as of September 22, 2026, GPT-5.5 Chat is the default standard-harness model. Before enabling any model tagged *cross-geo*, complete a data-residency review under [Control 2.7](../pillar-2-security/2.7-data-residency.md). Do not approve experimental or preview models for production use.
- [ ] Track major Copilot agent-platform updates through Message Center and the AI at Work Roadmap. When Microsoft marks a Copilot update as deferred-capable, document whether the agent validation group receives it through Standard Release while regulated production users remain on Deferred Release for the 30-day preparation window.
...
Export the published agents that can surface for Copilot users and record each in the Copilot-side approved list with owner, availability decision, and a reference to its FSI-AgentGov lifecycle record. Use the Agent registry path (**Agents > All Agents > Registry**) for tenant-wide visibility and review agent detail pages for ownership and availability exceptions. Do not duplicate authoring/testing/version detail — reference the authoritative FSI-AgentGov record.
...
### Step 4: Record the Model-Selection Decision

In Copilot Studio, record the selected primary model, its release type, and whether Microsoft marks it as cross-geo. As of September 22, 2026, GPT-5.5 Chat is the default standard-harness model; because the **Default** alias can be upgraded, retain the effective named model in validation evidence. Block experimental and preview models from production, route cross-geo models through Control 2.7, and require both Power Platform environment enablement and Microsoft 365 admin center provider approval before an external model is used.

### Step 4a: Review Release-Preference Impact

For Copilot agent-platform changes that Microsoft classifies as major and deferred-capable, record whether the affected users are in Standard Release or Deferred Release. Deferred Release delays eligible Copilot GA updates for 30 days after Standard Release begins; it does not replace Copilot Studio ALM testing, publishing approval, or the FSI-AgentGov lifecycle record.

### Step 5: Audit and Supervision Coverage
...
| 9 | Review primary model evidence | The effective named model, release type, cross-geo status, and any external-provider approvals are recorded; experimental/preview models are absent from production | Recommended |
...
- [Configure Standard and Deferred release options for Microsoft 365](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/configure-release-options)
- [Microsoft AI at Work Roadmap](https://www.microsoft.com/microsoft-365/roadmap)
