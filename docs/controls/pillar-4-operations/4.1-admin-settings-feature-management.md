...
Baseline Security Mode (BSM) is an opt-in, secure-by-default configuration that covers key Microsoft 365 services, including Microsoft 365 apps, SharePoint and OneDrive, Teams, Exchange Online, and the Microsoft Entra identity platform. BSM is relevant to Copilot governance as a complementary foundation, but it should not be documented as a Copilot-specific tab. Microsoft documents it as an organization-wide Microsoft 365 security baseline under **Settings > Org settings > Security & privacy > Baseline Security Mode**.
...
!!! note "BSM availability"
    Microsoft documents Baseline Security Mode as available in the Microsoft 365 admin center. For FSI teams, BSM is best treated as a complementary control surface that strengthens the broader Copilot security posture rather than replacing workload-specific Purview, SharePoint, or agent governance decisions. Verify release timing through Microsoft roadmap or release notes before asserting a specific GA month.

### AI Administrator Role and Copilot Administration Surfaces

Microsoft documents **AI Administrator** as the least-privilege role for managing Copilot-related settings in the Microsoft 365 admin center. Use the Copilot page in the admin center as the central hub for Copilot controls, and document which admin surfaces each AI Administrator can change:

- **Copilot settings** — User access, Data access, Copilot actions, and Other settings
- **Agents administration** — Agent inventory, lifecycle, user access, allowed types, and sharing
- **Security and data access controls** — Copilot security pivot and related Purview/SharePoint governance surfaces
- **Evidence collection** — Settings snapshots, role assignments, and audit logs for configuration changes

The **AI Administrator** role is the recommended RBAC role for Copilot management, separating day-to-day Copilot and agent administration from the broader privileges of Global Administrator or Entra Global Administrator. FSI organizations should assign this role to designated Copilot administrators to enforce least-privilege access and reduce the operational risk associated with over-privileged accounts.
...
| Microsoft Scout (Frontier preview) — admin gates | Partial | Scout's tenant-level admin surface is **Copilot > Settings > View all > search "Frontier" > Copilot Frontier**; enablement additionally requires Intune (or equivalent MDM) endpoint policy + admin attestation, and a per-user GitHub Copilot Business/Enterprise entitlement. Not all Scout controls live in the M365 admin center. See [Control 4.16](4.16-microsoft-scout-governance.md). |
...
!!! info "GA Feature — Requires Policy Assessment"
    ...
    Scheduled Prompts allow users to automate recurring Copilot interactions across Microsoft Teams, Office.com chat, and Microsoft Outlook. To manage scheduled prompts, admins need the Microsoft Copilot license in the Copilot subscription. Scheduled prompts are governed through the Cloud Policy setting **Allow the use of connected experiences in Office**; Microsoft documented that scheduled prompts move from Optional Connected Experiences to Connected Experiences effective August 2026.
...
- **Power Platform environment dependency:** The Microsoft 365 environment in Power Platform supports scheduled prompts and is automatically created when a user creates a scheduled prompt for the first time. The current scheduled-prompts feature no longer relies on Power Automate flows. Legacy scheduled prompts created during public preview continue to run in Power Automate until they expire and must be managed there.
- **Access control:** Administrators can control scheduled prompt availability via Cloud Policy by setting **Allow the use of connected experiences in Office**. FSI organizations should evaluate whether to restrict this capability during initial rollout, recognizing that disabling connected experiences also turns off other Microsoft 365 app features.
...
4. Coordinate with Power Platform governance to inventory the Microsoft 365 environment and any legacy Power Automate prompt flows
