...
| Microsoft Scout (Frontier preview) — admin gates | Partial | Scout's tenant-level admin surface is **Copilot > Settings > Frontier**; enablement additionally requires Intune (or equivalent MDM) endpoint policy + admin attestation, and a per-user GitHub Copilot Business/Enterprise entitlement. Not all Scout controls live in the M365 admin center. See [Control 4.16](4.16-microsoft-scout-governance.md). |
