...
**Regulatory Reference:** Sarbanes-Oxley §§302/404 (ITGC - Change Management), FFIEC IT Examination Handbook, GLBA §501(b)
...
**Sarbanes-Oxley §§302/404 (ITGC):** IT general controls for publicly traded financial institutions include change management controls. SOX auditors expect that changes to production systems -- including cloud-based services -- are evaluated, approved, tested, and documented before deployment. When Microsoft pushes a new Copilot feature that changes how data is processed or accessed, the institution needs a process to assess, approve, and document the change.
...
The M365 Message Center is the primary channel through which Microsoft communicates upcoming changes. Microsoft separates Message Center **Category** values from **Tag** values; both should feed the change register:

| Message Center signal | Source field | Description | FSI Action |
|-----------------------|--------------|-------------|-----------|
| Plan for change | Category | Upcoming changes to Microsoft 365 that may require action | Assess impact, plan governance response |
| Stay informed | Category | New or updated features and general service updates | Review for governance relevance |
| Prevent or fix issues | Category | Known issues and recommended mitigations | Assess impact, implement mitigations |
| Major update | Tag / major-update signal | Significant service changes communicated in advance when action may be required | Full impact assessment, CAB review |
| Retirement | Tag | Feature deprecation or end-of-life notice | Plan migration, update procedures |
| Deferred feature | Tag | Copilot major update eligible for Deferred Release handling | Evaluate Standard/Deferred audience assignment and validation window |
...
**Release Notes and What's-New Monitoring**

In addition to Message Center, Microsoft publishes Copilot changes through product release notes, what's-new pages, and the Microsoft AI at Work Roadmap, formerly known as the Microsoft 365 Roadmap. These sources often provide more detailed feature context than Message Center posts and may announce changes before or after the corresponding MC post.

| Source | URL / Location | Monitoring Cadence | FSI Action |
|--------|---------------|-------------------|------------|
| **Microsoft 365 Copilot what's new** | [What's new in Microsoft 365 Copilot](https://learn.microsoft.com/en-us/microsoft-365/copilot/release-notes) | Weekly | Review for new features affecting governance controls |
| **AI at Work roadmap — Copilot filter (formerly Microsoft 365 Roadmap)** | [AI at Work roadmap](https://www.microsoft.com/microsoft-365/roadmap) filtered to Copilot | Monthly | Identify upcoming features that may require pre-deployment governance review |
| **AI at Work roadmap (Dynamics 365, Power Platform, Copilot Studio)** | [AI at Work roadmap](https://aka.ms/AIatWorkRoadmap) | Monthly | Track Copilot Studio and Power Platform agent-platform changes that affect [Control 4.14](4.14-copilot-studio-agent-lifecycle.md); wave-based release plans are historical-only starting September 2026 |
| **Microsoft Purview what's new** | [What's new in Microsoft Purview](https://learn.microsoft.com/en-us/purview/whats-new) | Monthly | Identify changes to compliance controls (DLP, retention, eDiscovery, Communication Compliance) |
| **Microsoft 365 Admin Center release notes** | M365 Admin Center > Settings > Release notes (verify exact blade label in your tenant) | Weekly | Review admin-facing changes alongside Message Center posts |

**Recommended intake workflow:**

1. Assign a team member (or the same individual monitoring Message Center) to review what's-new and release notes sources weekly
2. Cross-reference announcements with existing Message Center posts — some features appear in release notes before or after MC posts
3. For features identified in release notes but not yet in Message Center, create a provisional change register entry marked as "Monitoring — No MC Post Yet"
4. Include release-note source URLs in the change register as supplementary evidence for audit trails
5. Flag Copilot model updates (which may not appear as discrete features in any source) for CAB awareness

!!! note "Power Platform release-plan migration"
    Microsoft states that Dynamics 365, Power Platform, and Dataverse release plans will no longer be published starting in September 2026. New capabilities now appear in the AI at Work roadmap, while existing release plans remain available for historical reference.

**Release Preference Management**

Microsoft provides legacy release management through targeted release:

| Ring | Description | FSI Use |
|------|-------------|---------|
| Targeted release for selected users | Features released to a defined group first | Governance pilot group for impact assessment |
| Targeted release for everyone | Features released to all users before Standard release | Not recommended for FSI; use selected users instead |
| Standard release | Default release track for all tenants | Production deployment after pilot validation |

Microsoft also provides modern Copilot release preferences for major updates through Frontier, Standard Release, and Deferred Release. Standard Release is the default GA channel. Deferred Release applies to Microsoft Copilot updates that Microsoft identifies as both major updates and deferred-capable in Message Center, delaying those GA features for up to 30 days for selected users or groups. Frontier remains a separate pre-GA opt-in program for early evaluation.

| Copilot release audience | Description | FSI Use |
|--------------------------|-------------|---------|
| Frontier program | Pre-GA Copilot features, preview and subject to change | Isolated evaluation only; do not treat as production evidence |
| Standard Release | Default GA rollout for supported Copilot updates | Pilot or early-validator audience when Deferred Release is the tenant default |
| Deferred Release | Deferred-capable Copilot major updates delayed up to 30 days after Standard GA | Validation window for regulated production users before broad exposure |
...
| Microsoft Scout (Frontier preview) | High -- preview capability, boundaries, permission model, admin-attestation flow, and endpoint policy schema may change | Critical -- Frontier scoping, Intune endpoint policy, admin attestation, and GitHub Copilot entitlement each change independently; treat every Scout preview change as a change-management event. See [Control 4.16](4.16-microsoft-scout-governance.md). |
...
- [ ] Configure targeted release or Copilot release preferences with a pilot group of 20-50 governance-aware users
...
- [ ] Implement pilot testing procedures through targeted release or Copilot Standard/Deferred release preferences before broad deployment
...
2. Configure email notifications:
   ...
   - Configure immediate notifications for Major update and Plan for change messages
...
1. Navigate to **M365 Admin Center > Settings > Org Setting > Organization profile > Release preferences**
...
For Copilot major updates that Microsoft identifies as deferred-capable, also navigate to **M365 Admin Center > Copilot > Settings > All Settings > Copilot release preferences: General Availability**, choose Standard Release or Deferred Release, and configure user or security-group exceptions for the validation group. Verify this path and the available release-preference controls in the target tenant before relying on them as audit evidence.
...
When a new feature arrives in the targeted release ring or the Copilot Standard/Deferred validation audience:
...
5. Make governance decision before broad production deployment
...
| 2 | Verify release-preference configuration | Pilot or validation group defined with appropriate membership through targeted release or Copilot Standard/Deferred release preferences |
...
| 5 | Verify pilot testing documentation for recent feature changes | Testing results available for features in targeted release or Copilot Standard/Deferred release preferences |
...
- [Microsoft 365 Release Tracks](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/release-options-in-office-365)
- [Configure modern release options for Microsoft Copilot](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/configure-release-options)
- [AI at Work roadmap](https://www.microsoft.com/microsoft-365/roadmap)
