...
Establish governance over **Microsoft Scout** — an endpoint-installed, agentic Copilot experience delivered through the Frontier preview program that can read and write local workspace files, execute shell commands, drive a browser through Playwright, call WorkIQ and other Microsoft 365 sources, invoke skills, subagents, MCP servers, and run scheduled or triggered automations. This control helps organizations decide whether, for whom, and under what supervision Scout is enabled, and it identifies where existing Microsoft 365 protections do and do not extend to Scout activity so adoption proceeds under documented governance rather than default entitlement.

## Why This Matters for FSI

Scout is delivered through Microsoft's **Frontier preview program** as a native application for **Windows 11 or later and macOS 12 or later**, and its capability surface is materially different from a browser-hosted Copilot experience. Scout operates on the endpoint: it can read and modify files in a user's workspace, execute shell commands (subject to per-command permission modes), navigate and interact with web pages through Playwright, reach into Microsoft 365 and WorkIQ, load skills and subagents, connect to Model Context Protocol (MCP) servers, and schedule or trigger automations that run on the user's behalf. Some inference is processed through **GitHub Copilot and third-party model providers**, under those providers' separate terms, and outside the Microsoft 365 data residency, retention, sensitivity-label enforcement, eDiscovery, and other M365 protections that FSI organizations rely on for the rest of the Copilot surface.

For regulated populations, legal, compliance, endpoint-security, third-party-risk, records, and ICFR owners should determine whether Scout creates supervision, books-and-records, third-party-risk, operational-risk, endpoint-control, ICFR, customer-information, or record-retention considerations for the firm's specific entity, workflow, and storage locations. Treat FFIEC, FINRA Rule 3110, Sarbanes-Oxley §§302/404, 12 CFR part 30, appendix D, GLBA §501(b), and SEC Rule 17a-4 as applicability prompts rather than conclusions that any requirement applies or is satisfied.
...
| **Microsoft 365 and WorkIQ access** | Retrieves and acts on content the user is already permitted to access | Existing oversharing risk is amplified by automated multi-step retrieval; WorkIQ scoping remains governed by its own controls |
...
| Microsoft 365 / WorkIQ data path | Partial | Scout reads and acts on Microsoft 365 content the user can already access; WorkIQ scoping is governed by its own controls |
...
**Broker-dealers:** Before enabling Scout for registered representatives, have legal/compliance determine whether Scout outputs that contribute to client communications or correspondence, session content in OneDrive, automation instructions, or locally stored MCP output fall within the firm's supervisory and books-and-records procedures, including any FINRA Rule 3110 or SEC Rule 17a-4 analysis.

**Banking institutions:** Treat Scout adoption as a change-control and third-party-risk assessment item; have risk/compliance determine whether FFIEC evidence is required for the specific entity, workflow, Frontier scoping, endpoint policy, admin attestation, GitHub entitlement, MCP-server approval decisions, or third-party inference boundary.

**SOX-reporting entities:** Where Scout is used in finance, reporting, or control-support workflows, have the ICFR owner determine whether configuration, approval evidence, local file-write behavior, or control-support workflow documentation belongs in the Sarbanes-Oxley §§302/404 evidence set.

**Covered institutions under OCC Heightened Standards:** Where applicable, have risk governance owners determine how Scout adoption, third-party inference, and local execution should be reflected in the firm's 12 CFR part 30, appendix D governance evidence.
