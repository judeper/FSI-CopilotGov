...
| Level | Requirement |
|-------|-------------|
| **Baseline** | Deploy the Triage Agent in manual, one-alert-at-a-time mode; review categorizations weekly; document as a model in the firm's model inventory |
| **Recommended** | Configure scheduled automatic runs after validation; Triage Agent recommendations guide investigator queue prioritization; quarterly review of categorization accuracy |
...
2. Configure the Triage Agent to run manually on one alert at a time initially (Baseline)
3. Review Triage Agent categorizations for the first 30 days to validate accuracy
4. After validation, configure scheduled automatic runs so Triage Agent recommendations can drive investigator queue order (Recommended)
