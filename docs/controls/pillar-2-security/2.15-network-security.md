...
| **Copilot-specific** | `copilot.microsoft.com`, `substrate.office.com`, `*.cloud.microsoft`, `*.svc.ms` (WebSocket) | Copilot API and processing | Yes |
...
!!! warning "WebSocket Requirements"
    Copilot real-time features (Pages, Notebooks, live collaboration) require WebSocket (wss://) connectivity to *.svc.ms, *.office.com, and *.cloud.microsoft. Firewalls and proxies must allow WSS traffic on port 443.
