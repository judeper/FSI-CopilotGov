...
- **FIPS Compliance:** FFIEC and NYDFS expect encryption implementations to meet recognized standards. Azure Key Vault Premium tier HSMs and Azure Key Vault Managed HSM are validated to FIPS 140-3 Level 3 using Marvell LiquidSecurity hardware; the Standard tier uses FIPS 140 Level 1 validated software cryptography. Organizations should confirm the current validation status for the specific HSM firmware deployed in their tenant, because validation levels are subject to periodic re-certification.
...
10. **FIPS Validation:** Confirm that encryption implementations use FIPS 140 validated cryptographic modules — Azure Key Vault Premium tier and Managed HSM are validated to FIPS 140-3 Level 3 (required for Regulated); Standard tier software cryptography is FIPS 140 Level 1 validated
