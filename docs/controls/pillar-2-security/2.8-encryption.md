...
- **OCC Bulletin 2023-17** and related guidance expects banks to maintain encryption standards for outsourced data processing — Copilot's cloud-based processing must meet these standards
...
- **Sarbanes-Oxley §§302/404** internal controls should address encryption key management — key management practices for Copilot-accessible content affect SOX control assessments
...
| TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA384 | 1.2 | Supported | Supported |
| TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256 | 1.2 | Supported | Supported |
...
!!! info "EXTRACT Usage Right"
    The EXTRACT right controls whether Copilot can process encrypted content. Removing EXTRACT from a sensitivity label blocks Copilot grounding on that content, even when users retain VIEW access. This is the recommended per-document mechanism for Copilot access control on encrypted files.
...
**Prerequisites:** Azure subscription, Azure Key Vault (Premium SKU for HSM), Entra Global Admin + Purview Compliance Admin roles
...
- **FIPS Compliance:** FFIEC and NYDFS expect encryption implementations to meet recognized standards. Only HSM-protected keys in the Azure Key Vault Premium tier inherit the service's FIPS 140-3 Level 3 validation; software-protected keys can also be created in the Premium tier and do not inherit that Level 3 validation. Azure Key Vault Managed HSM is also validated to FIPS 140-3 Level 3 using Marvell LiquidSecurity hardware, while the Standard tier uses FIPS 140 Level 1 validated software cryptography. Organizations should confirm that each Premium-tier key is HSM-protected and verify the current validation status for the specific HSM firmware deployed in their tenant, because validation levels are subject to periodic re-certification.
...
10. **FIPS Validation:** Confirm that encryption implementations use FIPS 140 validated cryptographic modules — only Azure Key Vault Premium-tier HSM-protected keys and Azure Key Vault Managed HSM inherit FIPS 140-3 Level 3 validation (required for Regulated); Premium-tier software-protected keys do not inherit Level 3 validation, and Standard-tier software cryptography is FIPS 140 Level 1 validated
...
- [Customer Key for Microsoft 365](https://learn.microsoft.com/en-us/purview/customer-key-overview)
- [Double Key Encryption (DKE)](https://learn.microsoft.com/en-us/purview/double-key-encryption)
- [Azure Key Vault Overview](https://learn.microsoft.com/en-us/azure/key-vault/general/overview)
- [Technical Reference Details About Encryption (TLS Cipher Suites)](https://learn.microsoft.com/en-us/microsoft-365/compliance/technical-reference-details-about-encryption)
- [TLS 1.2 Enforcement for Microsoft 365](https://learn.microsoft.com/en-us/purview/prepare-tls-1.2-in-office-365)
