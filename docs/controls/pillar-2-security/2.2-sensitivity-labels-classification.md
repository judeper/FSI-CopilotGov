...
!!! info "EXTRACT Permission"
    Removing the EXTRACT usage right from a sensitivity label's encryption settings prevents Copilot from processing that content, even when users can open it. This is the primary per-document mechanism for controlling Copilot access to labeled content. See also: BlockContentAnalysisServices advanced label setting.
...
- **Site-to-file label inheritance:** When a SharePoint site has a sensitivity label applied, files stored in that site inherit the site's label as a default. This enables rapid labeling coverage across large document libraries without requiring per-file SIT inspection
- **Scale advantage:** Organizations can reach high labeling volumes quickly by labeling sites and Teams and letting container labels flow down to files, reducing reliance on manual user labeling
- **User training shift:** Rather than training users on when to apply labels, this model trains users on how to request exceptions when the inherited label is too restrictive for a specific use case
- **Copilot governance impact:** Combined with [sensitivity labels for Teams, Groups, and sites](https://learn.microsoft.com/en-us/purview/sensitivity-labels-teams-groups-sites) that enforce site privacy and sharing controls, this model helps establish a baseline where Copilot grounding respects site-level classification without requiring every file to be individually labeled
...
Auto-labeling policies can now override manually applied labels for **files in SharePoint and OneDrive**. Previously, auto-labeling could only override manual labels on emails — if a user manually applied a lower-priority label to a SharePoint or OneDrive file, auto-labeling would not reclassify it even when the file content matched a higher-priority auto-labeling rule.

- **What changed:** Auto-labeling policies can now upgrade a manually applied label to a higher-priority label when file content triggers an auto-labeling rule in SharePoint or OneDrive
- **FSI impact:** This closes a significant governance gap where users could manually label a document containing MNPI or client PII with a lower classification (e.g., "Internal") and the auto-labeling engine would not correct it. With the override capability, auto-labeling rules that detect high-confidence SIT matches (such as bulk SSNs or MNPI keyword patterns) can now reclassify files to the appropriate sensitivity level regardless of the user's manual label choice
- **Configuration:** Enable the override setting within the auto-labeling policy: Microsoft Purview > Information Protection > Auto-labeling > [policy] > Settings > Allow overriding manually applied labels
- **Caution:** Organizations should verify that auto-labeling rules have a low false positive rate (<3%) before enabling override for manually applied labels, since incorrect auto-labeling upgrades may disrupt user access to files via encryption changes. Run in simulation mode first

### Permission Level Renames (April 2026)

Microsoft has renamed two sensitivity label encryption permission levels:

| Previous Name | New Name | Change in Permissions |
|--------------|----------|----------------------|
| Reviewer | Restricted Editor | No change — view and edit rights without save-as or export |
| Co-author | Editor | **"Save as, Export" permissions removed** from the Editor role |

- **FSI impact:** Organizations that previously relied on the "Co-author" permission level to allow users to save copies or export labeled content should review their label configurations. The renamed "Editor" role no longer includes Save as/Export permissions, which may affect workflows where users need to create derivative documents from labeled sources
- **Action required:** Audit existing sensitivity labels that use the Co-author (now Editor) permission level and verify that the removal of Save as/Export permissions aligns with the intended data handling behavior. If Save as/Export is required, adjust the label's custom permissions accordingly
...
| Teams meetings | Internal — General | No (may block adoption) — default labeling now supported for Teams meetings |
...
| Teams | Yes | Limited | Yes — default labeling now applies directly to Teams meetings | Yes |
...
- [Microsoft Purview Secure by Default deployment model](https://learn.microsoft.com/en-us/purview/deploymentmodels/depmod-secure-by-default-intro) — Microsoft's recommended alternative labeling deployment approach that derives file labels from site (container) labels, enabling rapid labeling scale with minimal user interaction
