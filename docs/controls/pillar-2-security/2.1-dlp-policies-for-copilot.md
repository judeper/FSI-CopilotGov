...
Deploy Data Loss Prevention (DLP) policies that target the **Microsoft 365 Copilot and Copilot Chat** location to restrict sensitive financial data in typed prompts, external web-search requests, and labeled files or emails used during Copilot grounding. This control supports GLBA §501(b) safeguard requirements and helps reduce unauthorized exposure of personally identifiable information (PII), account numbers, and other regulated data through AI-assisted interactions.
...
Microsoft Purview DLP includes **Microsoft 365 Copilot and Copilot Chat** as a dedicated location in DLP policy configuration. Selecting this location disables all other locations for that policy. Administrators can create three architecturally distinct rule types that govern different enforcement points in the Copilot interaction chain. Sensitivity-label and SIT conditions cannot be combined in one rule, but separate rules can exist in the same Copilot policy.

### DLP Policy Type 1: Label-Based Response Blocking

Configured in the **Microsoft 365 Copilot and Copilot Chat** DLP policy location. When a user's Copilot prompt causes grounding against a file or email that carries a sensitivity label matching the policy condition, Copilot does not process the item's content for the response. The item can still appear as a citation.
...
- **Primary use:** Exclude labeled content from Copilot processing and response grounding
...
- **Availability:** Preview and rolling out. Confirm that the capability is available in the tenant before relying on enforcement.
- **Supported Copilot surfaces:** Microsoft 365 Copilot, Copilot Chat, and Copilot in Word, Excel, and PowerPoint. Do not assume the same prompt-blocking behavior in Outlook, Teams, OneNote, or Loop.
- **User messaging caveat:** During preview, Word, Excel, and PowerPoint might not clearly identify organizational policy as the cause of the block even though Copilot does not return a response.

All three policy types **must each be configured as separate DLP rules**. Sensitivity-label and SIT conditions cannot be combined within a single rule, but may exist as separate rules within the same policy.
...
| What is scanned | Supported files and emails referenced by Copilot | The text the user types directly into Copilot | The text the user types directly into Copilot |
| Trigger | Sensitivity label on source content | SIT pattern match in prompt text | SIT pattern match in prompt text |
| Effect on Copilot | Excludes labeled item content from processing; the item can remain visible as a citation | Blocks the response; no internal or web grounding | Blocks external web search only; Copilot may still answer from permitted internal M365 data |
...
| Status | Generally available | Preview and rolling out | Verify current tenant availability |
| Configured via | Purview DLP > Create policy > Microsoft 365 Copilot and Copilot Chat | Purview DLP > Create policy > Microsoft 365 Copilot and Copilot Chat | Purview DLP > Create policy > Microsoft 365 Copilot and Copilot Chat |

### Direct File Upload Limitation

DLP evaluates the text typed directly into a prompt, but it does **not** inspect the contents of a file uploaded directly with that prompt. Label-based controls can govern supported files and emails that Copilot retrieves or that are open in supported Microsoft 365 apps, but organizations should not treat prompt-uploaded file content as covered by SIT prompt inspection. Use approved storage, sensitivity labels, endpoint controls, and user guidance to address this gap.
...
- **Complement to Copilot-location DLP:** The Copilot policy location covers the supported Microsoft 365 Copilot, Copilot Chat, Word, Excel, and PowerPoint scenarios described above; Endpoint DLP can add device/browser controls for web interactions
...
!!! info "Feature status varies by rule type"
    The **Microsoft 365 Copilot and Copilot Chat** policy location supports generally available label-based processing restrictions and preview capabilities such as SIT prompt blocking. Organizations should verify current tenant rollout before making an enforcement dependency part of a production control.
...
- **Real-time prompt evaluation:** For the prompt-blocking action, a matching typed prompt receives no Copilot response and is not used for internal or web searches.
- **Web search blocking behavior:** For the web-search-only action, Copilot blocks external web search but may still answer from permitted internal Microsoft 365 data. Validate the user message shown in each supported surface rather than assuming a classic DLP policy tip.
...
|-----------|-------------|-----------------|
| **Copilot Location** | Dedicated location for supported Microsoft 365 Copilot and Copilot Chat interactions; it cannot share a policy with other locations | Apply FSI-specific SIT and sensitivity-label rules to supported Copilot interactions |
...
| **User messages and notifications** | Copilot can display an organizational-policy or sensitive-information message; wording and clarity vary by surface and preview status | Educate users about data handling and validate the actual message shown in each supported client |
...
4. Confirm all other locations are disabled automatically; create separate policies for Exchange, SharePoint, OneDrive, Teams, or Devices
5. Add rule condition: "Content contains sensitivity label = Highly Confidential"
6. Action: Prevent Copilot from processing the content
7. Configure the available user notification and validate the message in each supported Copilot surface
...
1. Confirm SIT prompt blocking preview rollout has reached the tenant, then create a new DLP policy targeting the **Microsoft 365 Copilot and Copilot Chat** location
...
4. Configure user notifications where available and validate the actual organizational-policy or sensitive-information message shown by each supported client
...
1. **Three DLP Policy Types Evaluated:** Confirm at least one label-based response blocking rule exists with the Microsoft 365 Copilot and Copilot Chat location enabled; where tenant rollout permits, confirm a separate SIT-based prompt blocking rule; evaluate whether SIT-based web search restriction is appropriate for the organization's risk posture — verify in Microsoft Purview > DLP > Policies
...
10. **User Message Validation:** Confirm that each supported client blocks as configured and capture the organizational-policy or sensitive-information message shown; do not fail the control solely because preview clients do not display a classic policy-tip explanation
11. **Direct Upload Gap:** Confirm user guidance and complementary controls address files uploaded directly in prompts, whose contents are not scanned by Copilot DLP
12. **Custom SIT Accuracy:** Validate that custom FSI SITs (CUSIP, ISIN, CRD) correctly match intended patterns and do not produce excessive false positives (target <5% false positive rate)
13. **Periodic Review Evidence:** Confirm that DLP policy review cadence is documented (quarterly at minimum) and that review records are retained
