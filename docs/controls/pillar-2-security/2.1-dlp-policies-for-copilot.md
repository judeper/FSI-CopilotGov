...
These two policy types **must be configured as separate DLP rules** — they cannot be combined within a single DLP rule, but may exist as separate rules within the same policy.
...
| Can be combined with the other type | No — separate DLP rule required | No — separate DLP rule required |
...
!!! info "Preview status (Roadmap ID 548671)"
...
- [Create Custom Sensitive Information Types](https://learn.microsoft.com/purview/sit-create-a-custom-sensitive-information-type)
