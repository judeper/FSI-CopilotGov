...
| Microsoft Scout (Frontier preview) — MCP servers | Yes (via MCP) | Not applicable | Per MCP server | Scout connects to Model Context Protocol (MCP) servers as third-party extensibility; MCP output is stored on the local endpoint and is outside the Microsoft 365 DPA. Treat MCP servers as connectors under this control and under [Control 2.16](2.16-federated-connector-mcp-governance.md); apply the Scout-specific approval criteria (data path, authentication mechanism, external egress, per [Control 4.16](../pillar-4-operations/4.16-microsoft-scout-governance.md)). |
...
- [Microsoft Graph Connectors Overview](https://learn.microsoft.com/en-us/microsoft-365/copilot/extensibility/overview-copilot-connector?toc=%2Fgraph%2Ftoc.json)
