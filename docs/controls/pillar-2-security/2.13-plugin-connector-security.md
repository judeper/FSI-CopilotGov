...
```powershell
# Search documented plugin and Copilot interaction operations
Search-UnifiedAuditLog -StartDate (Get-Date).AddDays(-30) -EndDate (Get-Date) `
  -Operations "EnablePlugin","CopilotInteraction" -ResultSize 5000
...
`CopilotInteraction` is broader than plugin use. Inspect the `AISystemPlugin` property in each record's audit data to identify the plugin name, ID, and version; do not count every Copilot interaction as a plugin invocation.
