...
> **Agent 365 control-plane and agent-identity governance are authoritative in FSI-AgentGov.** This control governs the **Copilot-surface security guardrails** for declarative and SharePoint agents — who can use, share, and access agents that appear in the Microsoft 365 Copilot experience, configured through the M365 admin center Agents control plane. The deeper Agent 365 *control-plane* configuration (management rules, registry sync, agent map) and *agent identity* governance are specified in **[FSI-AgentGov Control 2.25 — Agent 365 Admin Center Governance Console](https://judeper.github.io/FSI-AgentGov/controls/pillar-2-management/2.25-agent-365-admin-center-governance-console/)** and **[FSI-AgentGov Control 2.26 — Entra Agent ID Identity Governance](https://judeper.github.io/FSI-AgentGov/controls/pillar-2-management/2.26-entra-agent-id-identity-governance/)**. Consume and cross-reference those controls rather than re-specifying control-plane mechanics here.

#### Agent Management Rules and Policy Templates

The Microsoft 365 admin center currently exposes five **Agent management rules** actions:

- **Install Microsoft agents**
- **Reassign ownerless Agent Builder agents to manager**
- **Block ownerless agents without usage**
- **Apply a template to agents**
- **Reject publish requests older than a specified number of days**

These product actions should not be replaced with inferred rule types such as publisher allowlisting, generic ownership requirements, lifecycle policies, or sharing-scope rules. Publisher categories, sharing, and user access are separate settings in the Agents control plane.

Policy templates remain relevant for applying predefined security policies to new agents. Separately, the **Apply template** management-rule action applies a preexisting or custom template to eligible existing published agents at scale. Administrators define criteria, select the template, review all eligible agents or a subset, and run the rule. The action evaluates Registry agents that have an agent identity; it does not apply to agent blueprints or AI teammates, and it is a one-time bulk operation rather than a scheduled rule.

Consistent with the authority boundary above, use FSI-AgentGov Controls 2.25 and 2.26 for the normative control-plane and agent-identity requirements. This control records only the technical distinction needed to avoid treating templates as universal or continuously scheduled enforcement.

#### Agent Map

The **Agent Map** in the Microsoft 365 admin center provides a visual inventory of tenant agents grouped by platform or builder, with filters such as status, publisher type, platform, channel, data source, and usage where supported. Summary cards help administrators focus on agents at risk, agents without owners, and unmanaged agents. The Single Agent Map preview can show user and tool connections for one selected agent based on Agent 365 observability data, but Microsoft Learn states that knowledge signals are not included yet.

**FSI application:** Agent Map helps institutions identify governance gaps such as ownerless, unmanaged, inactive, or high-risk agents, and the Single Agent Map preview can support investigation of selected-agent user activity and tool-call exceptions. Do not use Agent Map as evidence that agents share the same knowledge source, or that an agent is orphaned because of stale knowledge-source updates, unless a tenant export or future Microsoft source explicitly provides that signal.
...
Organizations should verify that Entra Agent ID is enabled for governed agents and that Conditional Access policies are reviewed to account for agent identities alongside human user identities. Microsoft Learn states that Microsoft Entra Agent ID is available for Microsoft Entra customers, that extending Entra security features to agents requires Microsoft Agent 365, and that Conditional Access for agents requires Microsoft Entra ID P1 or P2 plus a Microsoft Agent 365 license for each user, with Agent 365 license enforcement "coming soon." Treat licensing and enforcement as tenant evidence items before representing Conditional Access as active for agents. See [What is Microsoft Entra Agent ID](https://learn.microsoft.com/en-us/entra/agent-id/what-is-microsoft-entra-agent-id) and [Conditional Access for agents](https://learn.microsoft.com/en-us/entra/identity/conditional-access/agent-id).
...
| **Copilot Studio agents** (organizational) | 2.13 (plugin security), 2.14 (agent governance), 2.16 (MCP/connectors) | Full — visible in Agent 365 registry with ownership, usage analytics | Require Agent 365 registry enrollment and evaluate applicable policy templates; the **Apply template** action covers only Registry agents with an agent identity |
...
| **Agent management rules** | Runs one of the supported bulk actions; **Apply template** is one-time and limited to Registry agents with an agent identity | Agents > Settings > Agent management rules |
| **Security templates (policy templates)** | Defines reusable security policies for new agents; eligible existing published agents can receive a template through the separate **Apply template** action | Agents > Settings > Security templates |
...
| **Agent feedback sharing and tags** | Controls whether developers receive agent feedback and how admins/users label agents for discovery and organization | Agents > Settings > Agent feedback sharing / Tags |
