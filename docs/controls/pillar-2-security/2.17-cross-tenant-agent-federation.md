...
### Cross-Tenant Patterns (reference)

Cross-tenant agent patterns can include Entra Agent ID-backed agents, MCP / BYO MCP servers, Copilot Studio A2A endpoints, and externally published agents. The authoritative identity, trust, endpoint, and attestation controls are described in full in the FSI-AgentGov controls referenced above. From the Copilot side, each pattern reduces to the same question: *should this externally originated agent or endpoint be allowed to surface for the firm's Copilot users, and is it inventoried for supervision?* Verify the exact tenant control path for each pattern; do not assume one Entra cross-tenant access setting covers every A2A, MCP, or published-agent scenario without tenant evidence.
...
| Copilot Studio A2A connections | Yes | External A2A agents can be hosted outside Copilot Studio; Microsoft documents authentication options of None, API key, or OAuth 2.0, and A2A metadata can include full chat history |
...
### Step 1: Identify Externally Published Agents and Endpoints Reaching Copilot

**Portal:** Microsoft 365 admin center > Integrated apps and Microsoft 365 admin center > Agents (Agent registry).

Review which externally published agents, A2A endpoints, and MCP servers can surface for the firm's Copilot users. Record each one in the firm's Copilot-side approved list with owner, counterparty, endpoint/control path, authentication mode, and approval status. Deep identity, A2A, and MCP-server attributes are maintained in the FSI-AgentGov inventory — reference, do not duplicate, that record.
...
### Step 3: Configure Identity, Conditional Access, A2A, and MCP Trust in FSI-AgentGov
...
> **A2A payload note:** Microsoft Learn for Copilot Studio A2A connections states that typical A2A message metadata includes a `contextId`, message IDs, locale information, and full chat history, not just the latest user utterance. Treat each external A2A endpoint as a transcript-sharing boundary and capture endpoint authentication, data residency, and vendor-risk approval before enabling it. See [Connect an agent available over the Agent2Agent (A2A) protocol](https://learn.microsoft.com/en-us/microsoft-copilot-studio/add-agent-agent-to-agent).
...
| 2 | Confirm default-block posture in the Copilot surface | Externally published agents/endpoints are blocked or unavailable by default where the tenant exposes a control; only approved-list entries are admitted | Baseline |
...
| 8 | Verify external A2A transcript boundary | Each approved external A2A endpoint has evidence of authentication mode, expected payload metadata, full-chat-history handling, and vendor-risk/data-residency approval | Regulated |
...
- [Connect an agent available over the Agent2Agent (A2A) protocol](https://learn.microsoft.com/en-us/microsoft-copilot-studio/add-agent-agent-to-agent)
