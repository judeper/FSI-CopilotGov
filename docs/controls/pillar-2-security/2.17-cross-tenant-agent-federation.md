...
2. Create Conditional Access policies scoped to **All agent identities**, selected agent identities, or agent identity blueprints. Start in report-only mode, then enforce block / allow decisions for approved resources
3. Cover all three access patterns: on-behalf-of user, application-only (app-only) agent access, and agent user accounts
