...
|-------|-------------|-----------|
| **Baseline** | Review **Agents > Settings > Allowed agent types** and restrict the Microsoft-published and external-publisher categories used by federated connectors where the broader effect on agents and apps is acceptable; set each unapproved connector's allowed-user scope to **No users**; document the restriction rationale; monitor Message Center for tenant-specific action notices | Maximum restriction during initial governance assessment while recognizing that publisher-category controls affect more than connectors |
...
1. Review **Allowed agent types**. If the institution's Baseline posture requires a tenant-wide category restriction, deselect the Microsoft-published and external-publisher categories used by federated connectors only after assessing the effect on other agents and apps in those categories.
2. In **Copilot connectors > Your connections**, set every unapproved connector's allowed-user scope to **No users**. For an approved exception, assign only the intended users or groups and use **Staged rollout** where available.
...
|---|------------------|-----------------|------------------|
| 1 | Review **Allowed agent types** and connector-specific allowed-user scopes in M365 Admin Center | Publisher-category settings match the approved tenant posture, and every unapproved connector is scoped to **No users** | Baseline |
