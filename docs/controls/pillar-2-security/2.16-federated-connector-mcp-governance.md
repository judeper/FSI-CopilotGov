...
Establish governance controls for federated Copilot connectors using the Model Context Protocol (MCP) — a real-time data retrieval mechanism that is architecturally distinct from plugins and Graph connectors. Federated connectors authenticate with user credentials (not admin-managed service accounts) and retrieve live data from external sources without indexing. Microsoft-published connectors can be available by default subject to the tenant's **Allowed agent types** settings, while partner connectors require administrator approval. This control supports compliance with GLBA safeguard requirements, SEC Reg S-P privacy obligations, and FFIEC expectations for third-party data access governance.
...
!!! important "The tenant-wide PowerShell toggle is retired"
    ...
    Microsoft retired the `Set-FederatedConnectorToggle` command-line toggle on **August 25, 2026**. Do not use the cmdlet to configure or verify current effective access. Tenants that previously used it may receive a Message Center post with a tenant-specific window to reapply the choice in **Allowed agent types**; follow the notice for the affected tenant rather than assuming a universal reapplication deadline. After that window, Microsoft states that the former cmdlet state is no longer honored.
