...
Microsoft Defender XDR added agent protection capabilities (November 2025), extending XDR threat detection to Copilot agents and other AI agents deployed in the organization:
...
**Microsoft Security for AI** (previously "Microsoft Defender for AI") extends runtime protection to AI agents across Microsoft 365, Copilot Studio, and Power Platform:
...
| **Cross-platform coordination** | Extends runtime protection across M365 Copilot agents, Copilot Studio agents, and Power Platform agents | Provides unified security coverage regardless of agent authoring surface |

**Onboarding Security for AI:**

1. Navigate to Microsoft Defender portal > Settings > Security for AI
2. Enable real-time monitoring for the organization's agent inventory
3. Review and customize detection sensitivity thresholds (default thresholds may generate high false-positive rates during initial deployment)
4. Configure alert routing to the SOC and compliance teams
5. For Copilot Studio agents: coordinate with Power Platform administrators to enable Security for AI connectors in the Power Platform admin center
...
- Verify that Security for AI alerts appear in the Defender XDR unified incident timeline alongside correlated agent activity

**Power Platform coordination:** Copilot Studio agents require coordination between the M365 security team and Power Platform administrators. Security for AI policies defined in Defender must align with Power Platform DLP policies to avoid gaps where agent-generated data flows bypass one enforcement point but not the other.
...
- **Defender AI Agent Risk Inventory (February 2026):** Microsoft Defender now provides a risk-based inventory of AI agents, enabling security teams to assess and classify agent risk within the Defender portal. FSI organizations should integrate this inventory into their agent governance framework — it provides a native surface for identifying high-risk agents that access financial data, customer information, or regulated systems. Configure risk classification policies to flag agents that require additional security review under the organization's third-party risk management framework.
...
!!! info "Action Required — April–June 2026 Phased Rollout"

    Microsoft is auto-enabling Security Copilot for all M365 E5 customers in a phased rollout from April 20 to June 30, 2026. Tenants receive 400 Security Compute Units (SCUs) per 1,000 licensed users automatically — no opt-in is required. Microsoft provides a 7-day advance notice before activation.

**FSI governance implications:**

- **Auto-activation scope:** Security Copilot is embedded within Defender XDR, Entra, Intune, and Purview portals. E5 FSI tenants should expect autonomous AI capabilities in these products without explicit enablement.
- **Opt-out path:** Tenant owners can disable Security Copilot via **Owner Settings** in the Security Copilot portal before or after auto-activation. FSI organizations should evaluate whether to opt out during their governance review period.
- **Data processing:** Security Copilot processes security event data using Microsoft's AI models. Organizations should verify that data processing meets internal data governance and residency requirements (see [Control 2.7 — Data Residency](2.7-data-residency.md)).
...
- **Budget impact:** The 400 SCU allocation is included in E5 licensing, but overages may apply. Organizations should coordinate with [Control 4.8 — Cost Allocation](../pillar-4-operations/4.8-cost-allocation.md) to track SCU consumption.

**Recommended actions:**

1. Identify your tenant's activation date via Microsoft 365 Message Center
2. Evaluate opt-out vs. governance-with-guardrails approach
