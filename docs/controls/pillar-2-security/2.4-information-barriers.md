...
1. **Prevent BizChat from searching** across barrier-separated segments
...
|-------------|----------|----------------|---------|
| **Permanent** | Ongoing | BizChat cannot search across segments; Copilot cannot reference cross-segment files | Research vs. Investment Banking |
...
### BizChat Cross-Workload Search Implications

BizChat (Microsoft 365 Chat) performs a cross-workload search when answering user queries. Without information barriers, a single BizChat prompt could retrieve:

```
BizChat Query: "What's the latest on Acme Corp?"
         │
    ┌────┴────────────────────────────────────┐
    │            Without Barriers              │
    ├──────────────────────────────────────────┤
...
|-------------------|:---:|:---:|-------|
| BizChat (M365 Chat) | Yes | Yes | Critical — searches all workloads |
...
| Teams | Yes | Yes | Meeting summaries respect barriers |
...
| SharePoint (Agents) | Yes | Yes | Agents scoped to segment-accessible sites |
...
|-------|-------------|-----------|
| **Baseline** | Define organizational segments based on department attribute; create block policies between Investment Banking and Research; enable information barriers for Teams, SharePoint, and OneDrive; verify BizChat respects barriers | Minimum Chinese Wall implementation — addresses the most critical regulatory separation requirements |
| **Recommended** | Add Trading segment separation; implement temporary deal-specific barriers; enable barriers for all M365 workloads; quarterly barrier validation testing; compliance review of barrier configurations; integrate barrier events with audit logging | Comprehensive barrier strategy suitable for multi-service financial firms with investment banking, research, and trading operations |
| **Regulated** | All segments defined and enforced; automated barrier provisioning for new deals; real-time monitoring of barrier breach attempts; annual barrier effectiveness audit by compliance; supervisory access documented and reviewed quarterly; barrier configurations included in regulatory examination packages | Full Chinese Wall implementation for large, diversified financial institutions subject to frequent SEC/FINRA examinations |
...
2. Open BizChat and query for content that should be restricted (e.g., research reports)
...
3. **BizChat Barrier Enforcement:** As an Investment Banking user, query BizChat for research content — confirm no cross-segment results are returned
...
5. **Teams Barrier Enforcement:** Verify that Teams Copilot meeting summaries do not include content from barrier-separated participants
6. **SharePoint IB Mode:** Confirm SharePoint sites have appropriate IB modes applied and that Copilot in SharePoint respects barrier boundaries
7. **Temporary Barrier Activation:** Create a test temporary barrier and verify Copilot enforcement within 1 hour of policy application
8. **Supervisory Access:** Verify that Compliance segment users can access content across barriers as designed, and that access is logged
9. **Barrier Event Audit Logs:** Confirm that information barrier policy match events appear in the Unified Audit Log
10. **Periodic Validation:** Confirm quarterly barrier validation testing is scheduled, documented, and reviewed by compliance
