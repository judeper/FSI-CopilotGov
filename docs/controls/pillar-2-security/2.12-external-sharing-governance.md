...
Site-level values can override the organization-level setting under SharePoint admin center > Active sites > (site) > Settings > More sharing settings. Removal of the guest *account* from the directory is handled by the Entra ID access reviews configured in Step 4, where the action applied to denied guests can block sign-in and then remove the account from the tenant.
