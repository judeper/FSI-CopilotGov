...
- **NYDFS Part 500 (Section 500.12)** requires multi-factor authentication for any individual accessing internal networks from an external network — Copilot access from outside the corporate environment must require MFA at minimum. The June 2026 CA baseline-scopes enforcement change (see below) strengthens MFA enforcement for Copilot access from non-corporate environments as it rolls out progressively.
...
|------------|--------|-------------|
| Microsoft 365 Copilot (Enterprise Copilot Platform) | Look up the tenant's Application ID in Microsoft Entra; Microsoft Learn lists the app name but does not publish a GUID | Direct Copilot service |
...
> **Tenant verification required:** Microsoft Learn lists **Enterprise Copilot Platform** as an app included in the Conditional Access Office 365 app suite, but it does **not** publish a public authoritative GUID. Microsoft instructs administrators to look up application IDs in their own tenant by using Enterprise applications, sign-in logs, or Microsoft Graph PowerShell. Do not hard-code a copied GUID in policy automation or evidence; discover and record the tenant's current service principal Application ID before creating or auditing Copilot-specific CA policies.
>
...
### Conditional Access Enforcement Change (Baseline Scopes — Progressive Rollout Began June 15, 2026)

Microsoft Entra ID began a progressive rollout of an improved Conditional Access enforcement model on **June 15, 2026**, rolling out to tenants **progressively over several weeks**. As enforcement reaches a tenant, sign-ins that request only baseline scopes receive the same Conditional Access protections as other resource access. Because the rollout is progressive, enforcement may not yet be active in every tenant — tenants can also enable enforcement immediately or opt out per-policy ahead of the rollout via the [Baseline scopes settings](https://aka.ms/BaselineScopesSettingsUX).

**What changed:** Policies targeting "All resources" that include one or more resource exclusions now evaluate **baseline scopes** as directory access and subject them to Conditional Access — even for the excluded resources — when users sign in through client applications requesting only those baseline scopes. Baseline scopes are the OIDC scopes (`email`, `offline_access`, `openid`, `profile`) and baseline directory scopes (`User.Read`, `User.Read.All`, `User.ReadBasic.All`, `People.Read`, `People.Read.All`, `GroupMember.Read.All`, `Member.Read.Hidden`). Previously, these baseline scopes were automatically excluded from enforcement when a resource exclusion existed, creating a bypass path. Applications requesting any scope beyond the baseline (for example, `Mail.Read`) were already subject to enforcement and are unaffected.

**FSI impact:** Institutions with CA policies structured as "All resources + exclusions" should audit their policies for the baseline-scopes enforcement change now rolling out. If the tenant-discovered Enterprise Copilot Platform application is listed as an exclusion in any "All resources" policy, those policies may begin enforcing controls against baseline-scope Copilot sign-ins as the rollout reaches the tenant. Per NYDFS 23 NYCRR Part 500, Section 500.12, MFA requirements for external network access apply — this enforcement change closes a gap where exclusions relying only on baseline scopes could bypass MFA requirements.

**Rollout verification:** Use the [Baseline scopes settings](https://aka.ms/BaselineScopesSettingsUX) in Conditional Access to check the tenant's rollout status, enable enforcement early in a test tenant, or customize per-policy behavior. Run the Conditional Access optimization agent in Microsoft Entra ID to identify any remaining policies with resource exclusions. Review sign-in logs to confirm enforcement is behaving as expected as it reaches the tenant — confirm no unintended blocks or bypasses are present. Organizations that have not completed CA policy remediation should verify their current policy posture before enforcement reaches their tenant. Document remediation actions and current policy state for examination readiness.
...
| Microsoft Scout (Frontier preview) — sign-in surface | Yes | Yes | Yes | Yes | Sign-in to Scout resolves against Entra ID and can be scoped by Conditional Access; endpoint policy for Scout is deployed separately through Intune (Windows ADMX `AllowScoutFrontierAccess` or macOS `.mobileconfig`) and combined with an admin attestation, per [Control 4.16](../pillar-4-operations/4.16-microsoft-scout-governance.md). Scout's local shell, browser, and MCP-server capabilities on the endpoint are not directly governed by Conditional Access — endpoint controls apply. |
...
|-------|-------------|-----------|
| **Baseline** | Require MFA for all Copilot users; require managed device (Intune enrolled); block access from anonymous or Tor networks; audit CA policies for the June 2026 baseline-scopes enforcement change and remediate any Copilot exclusions from "All resources" policies; enable Adaptive Protection in audit mode | Minimum access controls — prevents Copilot access from unmanaged devices and unauthenticated sessions; baseline-scopes enforcement is rolling out progressively from June 15, 2026 |
| **Recommended** | Add phishing-resistant MFA (FIDO2, Windows Hello, certificate-based); enforce device compliance (not just enrollment); restrict to named corporate locations + approved VPN; 8-hour sign-in frequency; block medium/high risk sign-ins; confirm Copilot-specific exclusions were removed from "All resources" CA policies and verify enforcement behavior in sign-in logs as the baseline-scopes rollout reaches your tenant; enable Adaptive Protection dynamic blocking for high-risk users | Strong access posture suitable for most FSI firms — aligns with NYDFS Part 500 MFA requirements and confirms the CA exclusion bypass path is closed |
| **Regulated** | Require phishing-resistant MFA with authentication strength policy; compliant device from approved hardware list; corporate location only (no remote for Copilot); 4-hour sign-in frequency; real-time risk evaluation; integration with Defender for Cloud Apps for session-level monitoring; block all elevated risk sign-ins; enforce CA policies against all resources including Copilot with no exclusions, baseline-scopes enforcement verified as it rolls out; enable Adaptive Protection dynamic blocking at medium-risk threshold with mandatory investigation trigger | Maximum access restriction for highest-sensitivity environments — no Copilot exclusion gaps; immediate IRM-triggered access revocation for at-risk users |
...
### Step 2: Discover the Enterprise Copilot Platform App ID in Your Tenant

**Portal:** Microsoft Entra admin center > Entra ID > Enterprise applications

Before creating or modifying CA policies, discover the tenant's current Enterprise Copilot Platform service principal. Microsoft Learn confirms the app name is part of the Office 365 Conditional Access app suite and instructs admins to look up app IDs in their own tenants.

1. In **Enterprise applications**, set **Application type** to **Microsoft Applications**.
...
5. Use the discovered `AppId` when auditing CA include/exclude lists. If no service principal is returned, review Copilot sign-in logs for the application name or work with Microsoft support before creating policies that depend on direct Copilot app targeting.

### Step 3: Verify June 2026 Baseline-Scopes CA Enforcement Compliance

**Portal:** Microsoft Entra Admin Center > Protection > Conditional Access > Optimization

Verify that the June 2026 baseline-scopes CA enforcement change has been addressed in all CA policies:
...
3. **Target resources:** Enterprise Copilot Platform, using the Application ID discovered in your tenant; also include Microsoft 365 where broader suite coverage is required
...
# Search for policies referencing the tenant-discovered Microsoft-owned Copilot app ID
...
if (-not $copilotServicePrincipal) {
    throw "Enterprise Copilot Platform service principal was not found in this tenant. Verify in Enterprise applications or sign-in logs before auditing Copilot-specific CA policies."
}
...
- **NYDFS Part 500 Compliance:** Section 500.12 specifically requires MFA for accessing internal networks from external networks. For NYDFS-regulated entities, Copilot access from any non-corporate location must enforce MFA. The June 2026 baseline-scopes CA enforcement change closes a gap where "All resources + exclusion" policies could allow Copilot access without MFA — NYDFS-regulated institutions should verify this remediation as the rollout progresses. The Regulated governance level addresses this by restricting Copilot to corporate locations only.
...
- **Examination Readiness:** Maintain a Conditional Access policy inventory document showing all policies, their configurations, and last review dates. Include documentation of the June 2026 baseline-scopes enforcement readiness audit, remediation, and sign-in log review. Examiners may request this during IT examinations.

---

## Verification Criteria

1. **Tenant App ID Accuracy:** Verify all CA policies targeting Copilot reference the Enterprise Copilot Platform Application ID discovered in the tenant; retain the Enterprise applications or Graph PowerShell output as evidence and do not rely on a hard-coded GUID copied from documentation
2. **June 2026 Baseline-Scopes Enforcement Verification:** Confirm CA policy audit for "All resources + exclusion" patterns has been completed; confirm sign-in logs show expected behavior as enforcement rolls out; document remediation actions taken
...
- [Apps included in Conditional Access Office 365 app suite](https://learn.microsoft.com/en-us/entra/identity/conditional-access/reference-office-365-application-contents)
...
- [Improved enforcement for All resources policies with resource exclusions (baseline scopes)](https://learn.microsoft.com/entra/identity/conditional-access/concept-enforcement-resource-exclusions)
- [CA Enforcement Change — Improved enforcement for policies with resource exclusions (progressive rollout began June 15, 2026)](https://techcommunity.microsoft.com/blog/microsoft-entra-blog/upcoming-conditional-access-change-improved-enforcement-for-policies-with-resour/4488925)
