...
The Microsoft Purview eDiscovery experience consolidates discovery work into case-based workflows within the Microsoft Purview portal. All navigation in this walkthrough uses Microsoft Purview > eDiscovery > Cases.
...
> **Note:** For existing eDiscovery cases, verify that the selected data sources and search conditions cover Copilot activity, custodian mailboxes, and any in-scope SharePoint, OneDrive, Teams, and SharePoint Embedded containers. If Copilot activity or container locations are missing, add the documented sources manually and re-run affected searches or holds.
...
3. For each custodian, confirm the following locations are included:
   ...
   - Copilot activity (the unified Copilot content location -- confirm this is present)
...
| Copilot item-class filtering | Not required | Recommended | Required |
| Review set AI-assisted review | Optional | Recommended | Recommended |
| Existing case source verification | Required | Required | Required |
