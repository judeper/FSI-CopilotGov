...
- **Action required:** If your organization uses "All resources" CA policies with exclusions, audit and remediate for the June 15, 2026 baseline-scopes enforcement rollout (see Step 2)
...
Beginning June 15, 2026, Entra ID is progressively rolling out enforcement of Conditional Access for baseline scopes (OIDC scopes such as `openid`/`profile` and baseline directory scopes such as `User.Read`, `People.Read`) even for resources excluded in "All resources" policies. This closes a bypass path that previously allowed access requesting only those baseline scopes without full enforcement. The rollout reaches tenants progressively over several weeks; you can enable enforcement early or opt out per-policy via the [Baseline scopes settings](https://aka.ms/BaselineScopesSettingsUX).
...
| Tier | Recommendation |
|------|---------------|
| **Baseline** | Require MFA for all Copilot access; compliant device required; audit CA policies for Copilot exclusions for the June 2026 baseline-scopes rollout; enable Adaptive Protection in audit mode |
| **Recommended** | Location-based restrictions; session controls with 8-hour re-auth; remove Copilot-specific exclusions from "All resources" policies and test in report-only ahead of the June 2026 rollout; enable Adaptive Protection dynamic blocking for high-risk users |
| **Regulated** | Compliant device AND MFA AND trusted location required; session controls with 4-hour re-auth and app-enforced restrictions; Defender for Cloud Apps integration; CA policy remediation completed and enforcement enabled ahead of the June 2026 rollout; Adaptive Protection dynamic blocking at medium-risk threshold |
