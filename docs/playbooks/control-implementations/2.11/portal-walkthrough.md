...
> **Information Barriers Limitation:** Information Barriers are **not supported** for SharePoint Embedded content (including Copilot Pages and Notebooks). Organizations whose Legal/Compliance owners require IB-based ethical-wall or Chinese wall controls **must disable Pages and Notebooks creation** for affected populations or implement compensating controls, such as Cloud Policy scoping to exclude IB-affected groups. This limitation means IB-segmented users could potentially access cross-wall content through Pages if the feature is not restricted for those groups.
