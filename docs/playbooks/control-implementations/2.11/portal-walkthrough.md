...
> **Information Barriers Limitation:** Information Barriers are **not supported** for SharePoint Embedded content (including Copilot Pages and Notebooks). Organizations whose Legal/Compliance owners require IB-based ethical-wall controls should disable or scope Pages and Notebooks creation for affected populations, or document counsel-approved compensating controls. This limitation means IB-segmented users could potentially access cross-wall content through Pages if the feature is not restricted for those groups.
